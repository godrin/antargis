...
AGCharUtf8::AGCharUtf8(const AGCharUtf8 &p) : c(p.c), s(p.s), pos(p.pos) {}
...
bool AGCharUtf8::operator==(const AGCharUtf8 &p) const { return c == p.c; }
...
std::string AGCharUtf8::getString() const { return c; }

size_t AGCharUtf8::memory() const { return agFirstCharSize(c); }
...
AGStringUtf8::AGStringUtf8() { mLength = npos; }
...
size_t AGStringUtf8::memory() const { return s.length(); }
...
AGCharUtf8 AGStringUtf8::operator[](size_t i) { return AGCharUtf8(this, i); }
...
bool AGStringUtf8::operator==(const AGStringUtf8 &p) const { return s == p.s; }

bool AGStringUtf8::operator!=(const AGStringUtf8 &p) const { return s != p.s; }

bool AGStringUtf8::operator<(const AGStringUtf8 &p) const { return s < p.s; }

std::string AGStringUtf8::toString() const { return s; }
...
std::string AGStringUtf8::ascii() const {
  ...
  if (p != npos) {
    cdebug("WARNING: Probable problem within ascii for position "
           << p << " in string:" << s);
  }
  return s;
}

AGStringUtf8 AGStringUtf8::replace(const AGStringUtf8 &what,
                                   const AGStringUtf8 &by) const {
  size_t i = 0;
  size_t oi = 0;
  AGStringUtf8 n;
...
int AGStringUtf8::toInt() const { return atoi(s.c_str()); }
long AGStringUtf8::toLong() const { return atol(s.c_str()); }
float AGStringUtf8::toFloat() const { return atof(s.c_str()); }
...
std::string AGStringUtf8::get(size_t pos) const { return agSubStr(s, pos, 1); }
...
void AGStringUtf8::set(size_t i, const AGCharUtf8 &c) { operator[](i) = c; }
