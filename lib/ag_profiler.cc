...
double volatile Uint64 get_time_unit() {
  unsigned int a = 0, b = 0;
#ifdef RDTSC_TIMER
#if defined(__GNUC__)
  asm("RDTSC" : "=a"(a), "=d"(b) :);
#elif defined(_MSC_VER)
  __asm RDTSC __asm mov a, eax __asm mov b,
      edx
#endif
#endif
...
static std::string getStackNodeString(const SOURCE_LINE *stNode) {
  ...
  std::stringstream s;
  s << stNode->file << ", line " << stNode->line
    << " fn: " << stNode->funct + strName
    << " time: " << convert_time(stNode->time) << " count: " << stNode->count;
  if (stNode->count > 0)
...
public:
  ...
  void operator()(const SOURCE_LINE *l) { mAll += getStackNodeString(l); }
  operator std::string() { return mAll; }
};
...
std::string UserStackTraceHelper::getProfilerString() {
  ...
  std::copy(used_functions.begin(), used_functions.end(),
