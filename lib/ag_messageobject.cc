...
// AGEvent
AGEvent::AGEvent(AGListener *pCaller, const AGString &pName, const SDL_Event &e)
    : mCaller(pCaller), mName(pName), mEvent(e), mMousePosition(0),
      mRelMousePosition(0) {
  mClipped = false;
}
AGEvent::~AGEvent() {
...
bool AGEvent::isMouseEvent() const {
  return (mEvent.type == SDL_MOUSEMOTION || mEvent.type == SDL_MOUSEBUTTONUP ||
          mEvent.type == SDL_MOUSEBUTTONDOWN);
}

bool AGEvent::isClipped() const { return mClipped; }
void AGEvent::setClipped(bool f) { mClipped = f; }

void AGEvent::setVector(const AGVector2 &v) { mVector = v; }
...
AGString AGEvent::getName() const { return mName; }

void AGEvent::setName(const AGString &n) { mName = n; }

bool AGEvent::isSDLEvent() const { return eventOk(mEvent); }

/// returns the caller of this event
AGListener *AGEvent::getCaller() const { return mCaller; }
...
AGVector2 AGEvent::getMousePosition() const {
  ...
  case SDL_MOUSEMOTION:
    p[0] = mEvent.motion.x * getMain()->getVideo()->width() /
           getMain()->getVideo()->realWidth();
    p[1] = mEvent.motion.y * getMain()->getVideo()->height() /
           getMain()->getVideo()->realHeight();
    break;

  case SDL_MOUSEBUTTONUP:
  case SDL_MOUSEBUTTONDOWN:
    p[0] = mEvent.button.x * getMain()->getVideo()->width() /
           getMain()->getVideo()->realWidth();
    p[1] = mEvent.button.y * getMain()->getVideo()->height() /
           getMain()->getVideo()->realHeight();
    break;
  }
  return p;
...
/*
   AGMouseEvent::AGMouseEvent(AGListener *pCaller,SDL_Event
   *pEvent):AGEvent(pCaller)
   {
   }
...
bool AGListener::signal(AGEvent *m) { return false; }
...
/**
  This is the simple connect version. You have to override signal(.) in
  AGListener and provide a reasonable functionality there. For simple usage use
  AGSignal::connect( AGCPPListener *pListener);
...
AGMessageObject::AGMessageObject()
    : sigActive(this, "sigActive"), sigKeyDown(this, "sigKeyDown"),
      sigKeyUp(this, "sigKeyUp"), sigMouseMotion(this, "sigMouseMotion"),
      sigMouseButtonDown(this, "sigMouseButtonDown"),
      sigMouseButtonUp(this, "sigMouseButtonUp"), sigQuit(this, "sigQuit"),
      sigSysWM(this, "sigSysWM"), sigVideoResize(this, "sigVideoResize"),
...
void AGMessageObject::popSignal(AGSignal *pSignal) { mSignals.erase(pSignal); }
...
bool AGMessageObject::processEvent(AGEvent *agEvent) {
  ...
  if (rc) {
    ...
    if (false) {
      if (w)
        cdebug(toString(&agEvent->get())
               << " eaten up by " << typeid(*this).name() << " "
               << w->getName());
      else
        cdebug(toString(&agEvent->get())
               << " eaten up by " << typeid(*this).name()); //<<" "<<getName());
    }
  }
...
bool AGMessageObject::eventActive(AGEvent *m) { return false; }
bool AGMessageObject::eventKeyDown(AGEvent *m) { return false; }
bool AGMessageObject::eventKeyUp(AGEvent *m) { return false; }
bool AGMessageObject::eventMouseMotion(AGEvent *m) { return false; }
bool AGMessageObject::eventMouseButtonDown(AGEvent *m) { return false; }
bool AGMessageObject::eventMouseButtonUp(AGEvent *m) { return false; }
bool AGMessageObject::eventQuit(AGEvent *m) { return false; }
bool AGMessageObject::eventQuitModal(AGEvent *m) { return false; }
bool AGMessageObject::eventSysWM(AGEvent *m) { return false; }
bool AGMessageObject::eventResize(AGEvent *m) { return false; }
...
bool AGMessageObject::acceptEvent(const SDL_Event *pEvent) { return true; }

AGEvent *newEvent(AGListener *pCaller, const AGString &pName,
                  const SDL_Event &s) {
...
AGString toString(const SDL_Event *pEvent) {
  ...
  if (pEvent) {
    switch (pEvent->type) {
    case SDL_ACTIVEEVENT:
      os << "SDL_ACTIVEEVENT:" << (int)pEvent->active.gain << ":"
         << (int)pEvent->active.state;
      break;
    case SDL_KEYDOWN:
      os << "SDL_KEYDOWN:" << (int)pEvent->key.which << ":"
         << (int)pEvent->key.state << ":" << toString(pEvent->key.keysym);
      break;
    case SDL_KEYUP:
      os << "SDL_KEYUP:" << (int)pEvent->key.which << ":"
         << (int)pEvent->key.state << ":" << toString(pEvent->key.keysym);
      break;
    case SDL_MOUSEMOTION:
      os << "SDL_MOUSEMOTION:" << (int)pEvent->motion.which << ":"
         << (int)pEvent->motion.state << ":" << pEvent->motion.x << ":"
         << pEvent->motion.y << ":" << pEvent->motion.xrel << ":"
         << pEvent->motion.yrel;
      break;
    case SDL_MOUSEBUTTONDOWN:
      os << "SDL_MOUSEBUTTONDOWN:" << (int)pEvent->button.which << ":"
         << (int)pEvent->button.button << ":" << (int)pEvent->button.state
         << ":" << pEvent->button.x << ":" << pEvent->button.y;
      break;
    case SDL_MOUSEBUTTONUP:
      os << "SDL_MOUSEBUTTONUP:" << (int)pEvent->button.which << ":"
         << (int)pEvent->button.button << ":" << (int)pEvent->button.state
         << ":" << pEvent->button.x << ":" << pEvent->button.y;
      break;
    case SDL_JOYAXISMOTION:
      os << "SDL_JOYAXISMOTION:" << (int)pEvent->jaxis.which << ":"
         << (int)pEvent->jaxis.axis << ":" << pEvent->jaxis.value;
      break;
    case SDL_JOYBALLMOTION:
      os << "SDL_JOYBALLMOTION:" << (int)pEvent->jball.which << ":"
         << (int)pEvent->jball.ball << ":" << pEvent->jball.xrel << ":"
         << pEvent->jball.yrel;
      break;
    case SDL_JOYHATMOTION:
      os << "SDL_JOYHATMOTION:" << (int)pEvent->jhat.which << ":"
         << (int)pEvent->jhat.hat << ":" << (int)pEvent->jhat.value;
      break;
    case SDL_JOYBUTTONDOWN:
      os << "SDL_JOYBUTTONDOWN:" << (int)pEvent->jbutton.which << ":"
         << (int)pEvent->jbutton.button << ":" << (int)pEvent->jbutton.state;
      break;
    case SDL_JOYBUTTONUP:
      os << "SDL_JOYBUTTONUP:" << (int)pEvent->jbutton.which << ":"
         << (int)pEvent->jbutton.button << ":" << (int)pEvent->jbutton.state;
      break;
    case SDL_QUIT:
      os << "SDL_QUIT:";
...
void resetEvent(SDL_Event &pEvent) { pEvent.type = SDL_NOEVENT; }
