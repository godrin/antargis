...
TTF_Font *getFont(std::string s, int i) {
  ...
  if (!fontEngineFonts[info]) {
    ...
    if (!fontEngineInited) {
      ...
      if (TTF_Init() < 0) {
        //    TRACE;
        std::cerr << "Some error occured during initialization of libsdl-TTF"
                  << std::endl;
        return 0;
      }
      atexit(TTF_Quit);
    ...
    if (!mFont) {
      std::cerr << " Font not found:" << s << " probable file:" << name
                << std::endl;
...
AGTexture *AGFontEngine::renderText(int BaseLineX, int BaseLineY,
                                    const AGStringUtf8 &pText,
                                    const AGFont &pFont) {
  if (pText.length() == 0)
    return 0;
  ...
  if (fontCache.find(std::make_pair(pFont, pText)) == fontCache.end()) {
    ...
    //      if(false)
    {
      if (pFont.getBorder() > 0 || pFont.getEmbossed()) {
        ...
        copy.blit(as, AGRect2(move, move, as.width(), as.height()),
                  as.getRect(), AGColor(0xFF, 0xFF, 0xFF, 0xFF));
        as = copy;
        border(as, pFont.getBorderColor());
      } else {
        // simply copy, so there are no more problems on PPC
        AGSurface copy(as.width(), as.height());
        copy.blit(as, as.getRect(), as.getRect(),
...
AGFontEngine *getFontEngine() { return getScreen().getFontEngine(); }
