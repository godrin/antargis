...
void AGClipping::include(const AGRect2 &r) {
  ...
  for (std::vector<AGRect2>::iterator i = mExclude.begin(); i != mExclude.end();
       i++) {
...
std::vector<std::pair<AGRect2, AGRect2>> AGClipping::clip(const AGRect2 &r,
                                                          const AGRect2 &sync) {
...
AGString AGClipping::toString() const {
  ...
  os << "[AGClipping:";
  for (std::vector<AGRect2>::const_iterator i = mExclude.begin();
       i != mExclude.end(); i++)
    os << i->toString() << ";";
  os << "]";
