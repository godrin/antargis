...
void fillPoly(const std::vector<AGVector2> &pVecs, const AGColor &pColor) {
  ...
  // turned
  glBegin(GL_POLYGON);
  for (std::vector<AGVector2>::const_iterator i = pVecs.begin();
       i != pVecs.end(); i++) {
    glVertex2fv(*i);
  }
  glEnd();
...
void drawPoly(const std::vector<AGVector2> &pVecs, const AGColor &pColor,
              float pWidth) {
  assertGL;
  STACKTRACE;
...
void blit(const AGTexture &pSource, const AGRect2 &pRect, const AGRect2 &pSrc,
...
void blit(const AGTexture &pSource,
  ...
  for (std::vector<std::pair<AGRect2, AGRect2>>::const_iterator i =
    ...
    AGRect2 s(i->first.x() * itw, i->first.y() * ith, i->first.w() * itw,
              i->first.h() * ith);
    AGRect2 d(i->second);
...
void tile(const AGTexture &pSource, const AGRect2 &pTarget,
          const AGColor &pColor) {
  assertGL;
  STACKTRACE;
  AGRenderContext context;
  ...
  AGRect2 s(0, 0, float(pTarget.width()) / pSource.width(),
            float(pTarget.height()) / pSource.height());
...
void drawGradientAlpha(const AGRect2 &pRect, const AGColor &ul,
...
void drawBorder(const AGRect2 &rect, int W, const AGColor &c1,
                const AGColor &c2) {
  assertGL;
  STACKTRACE;
...
void blitTri(const AGTexture &pSource, const AGTriangle2 &pSrc,
...
void blit3dTri(const AGTexture &pSource, const AGTriangle2 &pSrc,
  ...
  AGProjection2D p(const_cast<AGTexture &>(pSource).glTexture()->getRect(),
                   AGRect2(0, 0, 1, 1));
