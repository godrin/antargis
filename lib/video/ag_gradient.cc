...
void drawGradient(SDL_Surface *surface, const AGRect2 &prect, const AGColor &ul,
                  const AGColor &ur, const AGColor &dl, const AGColor &dr) {
  AGRect2 rect = prect;
  Sint32 v00, v01, v02;
  Sint32 v10, v11, v12;
  ...
  Uint8 *bits = ((Uint8 *)surface->pixels) + int(rect.y() + oy) * pitch +
                int(rect.x() + ox) * bpp;
  ...
  for (register Sint32 x = 0; x < int(drawrect.w()); x++) {
    ...
    for (register Sint32 y = 0; y < int(drawrect.h()); y++) {
      ...
      case 2:
        pixel = (r >> Rloss) << Rshift | (g >> Gloss) << Gshift |
                (b >> Bloss) << Bshift;
...
void drawGradientAlpha(SDL_Surface *surface, const AGRect2 &prect,
                       const AGColor &ul, const AGColor &ur, const AGColor &dl,
                       const AGColor &dr) {
  AGRect2 rect = prect;
  ...
  for (register Sint32 x = 0; x < (int)drawrect.w(); x++) {
    ...
    for (register Sint32 y = 0; y < (int)drawrect.h(); y++) {
      ...
      case 3: {
        ...
        *bb = (*bb * (255 - a) + bi * a) >> 8;
        /*
         *((bits) + surface->format->Rshift / 8) = *((bits) +
      ...
      case 4:
        ...
        pixel = (ar >> Rloss) << Rshift | (ag >> Gloss) << Gshift |
                (ab >> Bloss) << Bshift;
...
void drawBorder(SDL_Surface *surface, const AGRect2 &rect, int width,
                const AGColor &c1, const AGColor &c2) {
  AGRect2 r = rect;
  ...
  for (int i = 0; i < width; i++) {
    sge_HLine(surface, (Sint16)r.x(), (Sint16)(r.x() + r.w() - 1),
              (Sint16)r.y(), uc1);
    sge_HLine(surface, (Sint16)r.x(), (Sint16)(r.x() + r.w() - 1),
              (Sint16)(r.y() + r.h() - 1), uc2);
    sge_VLine(surface, (Sint16)r.x(), (Sint16)r.y(),
              (Sint16)(r.y() + r.h() - 1), uc1);
    sge_VLine(surface, (Sint16)(r.x() + r.w() - 1), (Sint16)r.y(),
              (Sint16)(r.y() + r.h() - 1), uc2);
    r = r.shrink(1);
  }
}
/*
void AGDraw::drawGradient(AGScreen *surface, const AGRect2& rect, const AGColor&
ul, const AGColor& ur, const AGColor& dl, const AGColor& dr)
{
surface->drawGradientAlpha(rect,ul,ur,dl,dr);
}
void AGDraw::drawGradientAlpha(AGScreen *surface, const AGRect2& rect, const
AGColor& ul, const AGColor& ur, const AGColor& dl, const AGColor& dr)
{
surface->drawGradientAlpha(rect,ul,ur,dl,dr);
}
void AGDraw::drawGradientAlpha2(AGScreen *surface, const AGRect2& rect, const
AGColor& ul, const AGColor& ur, const AGColor& dl, const AGColor& dr)
{
surface->drawGradientAlpha(rect,ul,ur,dl,dr);
}
void AGDraw::drawBorder(AGScreen *surface, const AGRect2& rect,int width, const
AGColor& c1, const AGColor& c2)
{
