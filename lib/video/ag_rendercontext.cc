...
AGRenderContext::AGRenderContext()
    : mColor(0), mTexture(0), mProgram(0), mLighting(false), mDepthWrite(true),
      mDepthTest(true), mAlphaThres(-1), mAlpha(GL_NONE), mCulling(true) {
  getSurfaceManager()->registerMe(this);
}

AGRenderContext::AGRenderContext(const AGRenderContext &c)
    : mColor(c.mColor ? new AGVector4(*c.mColor) : 0), mTexture(c.mTexture),
      mProgram(c.mProgram), mLighting(c.mLighting), mDepthWrite(c.mDepthWrite),
      mDepthTest(c.mDepthTest), mAlphaThres(c.mAlphaThres), mAlpha(c.mAlpha),
      mCulling(c.mCulling) {
  getSurfaceManager()->registerMe(this);
}
...
AGRenderContext *AGRenderContext::getCurrent() { return &gCurrentContext; }
...
void AGRenderContext::setColor(const AGColor &pColor) {
  ...
  float f = 1.0 / 255.0f;
  mColor =
      new AGVector4(pColor.r * f, pColor.g * f, pColor.b * f, pColor.a * f);
}

void AGRenderContext::setTexture(AGGLTexture *pTexture) { mTexture = pTexture; }

void AGRenderContext::setLighting(bool pLight) { mLighting = pLight; }

void AGRenderContext::setDepthWrite(bool w) { mDepthWrite = w; }
void AGRenderContext::setDepthTest(bool w) { mDepthTest = w; }
void AGRenderContext::setCulling(bool c) { mCulling = c; }
void AGRenderContext::setAlpha(float v, GLuint g) {
  mAlpha = g;
  mAlphaThres = v;
}

AGGLTexture *AGRenderContext::getTexture() { return mTexture; }

AGVector4 *AGRenderContext::getColor() { return mColor; }
