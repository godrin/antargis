...
AGGLTexture::AGGLTexture(size_t W, size_t H, GLint format)
    : w(W), h(H), d(1), m3d(false), mRectTex(false), mTarget(GL_TEXTURE_2D) {
  assertGL;
  // cdebug("HUPE");
  initDMABuffer();
  assertGL;
  ...
  assertGL;
  glTexImage2D(mTarget, 0, format, w, h, 0, GL_RGBA, GL_UNSIGNED_BYTE, buffer);
  assertGL;
...
AGGLTexture::AGGLTexture(size_t W, size_t H, size_t D, GLint format)
    : w(W), h(H), d(D), m3d(true), mRectTex(false), mTarget(GL_TEXTURE_3D) {
  assertGL;
  getSurfaceManager()->registerMe(this);
  assertGL;
  ...
  glTexImage3D(mTarget, 0, format, w, h, d, 0, GL_RGBA, GL_UNSIGNED_BYTE, 0);
  assertGL;
...
void AGGLTexture::setSurface(AGInternalSurface *pSurface,
                             const AGVector2 &offset) {
  assertGL;
  GLint format;
  ...
  if (m3d) {
    ...
    glTexImage3D(mTarget, 0, format, w, h, d, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                 surface->pixels);
    glTexSubImage3D(mTarget, 0, int(offset[0]), int(offset[1]), 0, mw, mw, mh,
                    format, GL_UNSIGNED_BYTE, surface->pixels);
  } else {
    assertGL;

    glTexSubImage2D(mTarget, 0, int(offset[0]), int(offset[1]), surface->w,
...
GLuint AGGLTexture::id() { return mID; }
bool AGGLTexture::is3d() const { return m3d; }

size_t AGGLTexture::width() const { return w; }
size_t AGGLTexture::height() const { return h; }
size_t AGGLTexture::depth() const { return d; }

AGRect2 AGGLTexture::getRect() const { return AGRect2(0, 0, w, h); }
...
AGSurface AGGLTexture::getSurface() const {
  ...
  glGetTexImage(mTarget, 0, GL_RGBA, GL_UNSIGNED_INT_8_8_8_8_REV, pixels);
