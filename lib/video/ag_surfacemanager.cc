...
AGSurfaceManager::~AGSurfaceManager() {
  for (std::set<AGInternalSurface *>::iterator i = mSDLSurfaces.begin();
       i != mSDLSurfaces.end(); ++i) {
    AGFreeSurface((*i)->surface);
    checkedDelete(*i);
  }
  for (std::set<AGGLTexture *>::iterator i = mGLTextures.begin();
       i != mGLTextures.end(); ++i) {
    checkedDelete(*i);
  }
...
void AGSurfaceManager::clear() { cleanup(true); }
...
void AGSurfaceManager::registerMe(AGRenderContext *p) { mRContext.insert(p); }
void AGSurfaceManager::deregisterMe(AGRenderContext *p) { mRContext.erase(p); }

void AGSurfaceManager::registerMe(AGGLTexture *p) { mGLTextures.insert(p); }
void AGSurfaceManager::deregisterMe(AGGLTexture *p) { mGLTextures.erase(p); }
...
cdebug("rts:"<<mRContext.size());
  ...
  //   (i)   iterate through AGTextures & AGSurfaces and get all used
  //   AGInternalSurfaces (ii)  delete unused
...
#warning "// ALSO FOR SDL-MODE!!"
  ...
  for (std::set<AGTexture *>::iterator i = mTextures.begin();
       i != mTextures.end(); i++) {
    if ((*i)->textureUsed()) {
  ...
  for (std::set<AGRenderContext *>::iterator i = mRContext.begin();
       i != mRContext.end(); i++) {
    if ((*i)->getTexture())
  ...
  mGLTextures.clear();
  std::copy(gls.begin(), gls.end(),
            std::inserter(mGLTextures, mGLTextures.begin()));
...
size_t AGSurfaceManager::getUsedTexMem() const {
  size_t texMem = 0;
  for (std::set<AGGLTexture *>::const_iterator i = mGLTextures.begin();
       i != mGLTextures.end(); i++)
    texMem += (*i)->width() * (*i)->height() * (*i)->depth() * 4;
  return texMem;
}
