...
/* This code snippet was from Darren Grant sdl@lokigames.com, taken from
   ...
   As he published it to be imported into SDL_image, it seems to be ok to
   include it here. Changes: I removed some goto's.
 */
  ...
  if (info_ptr == NULL) {
    ...
    if (false) // setjmp(png_ptr->jmpbuf))
    {
      ...
      if (colortype & PNG_COLOR_MASK_PALETTE) {
        //                palette =
        //                (png_colorp)malloc(surface->format->palette->ncolors *
        //                sizeof (png_color));
        palette = (png_colorp) new char[surface->format->palette->ncolors *
                                        sizeof(png_color)];
...
SDL_Surface *fromPNG(std::string s) {
  SDL_Surface *surface =
      IMG_LoadPNG_RW(SDL_RWFromMem(const_cast<char *>(s.c_str()), s.length()));
  assertGL;
  return surface;
}
