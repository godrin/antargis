...
AGClipPaintTarget::AGClipPaintTarget(AGPaintTarget *pTarget)
    : mTarget(pTarget) {}

AGClipPaintTarget::~AGClipPaintTarget() throw() {}

void AGClipPaintTarget::blitTri(const AGTexture &pSource,
                                const AGTriangle2 &pSrc,
                                const AGTriangle2 &pDest) {
  cdebug("NOT IMPLEMENTED!");
}

void AGClipPaintTarget::blit(const AGTexture &pSource, const AGRect2 &pDest,
                             const AGRect2 &pSrc, const AGColor &pColor) {
  std::vector<std::pair<AGRect2, AGRect2>> l = mClipping.clip(pDest, pSrc);
  for (std::vector<std::pair<AGRect2, AGRect2>>::iterator i = l.begin();
       i != l.end(); i++) {
...
void AGClipPaintTarget::blit(const AGSurface &pSource, const AGRect2 &pDest,
                             const AGRect2 &pSrc) {
  std::vector<std::pair<AGRect2, AGRect2>> l = mClipping.clip(pDest, pSrc);
  for (std::vector<std::pair<AGRect2, AGRect2>>::iterator i = l.begin();
       i != l.end(); i++) {
...
void AGClipPaintTarget::drawLine(const AGVector2 &p0, const AGVector2 &p1,
                                 const AGColor &c) {
...
void AGClipPaintTarget::fillRects(
    const std::vector<std::pair<AGRect2, AGVector4>> &pr) {
...
void AGClipPaintTarget::blit(
  ...
  // collect clippings
  for (std::vector<std::pair<AGRect2, AGRect2>>::const_iterator i =
           pSrcDestRects.begin();
       i != pSrcDestRects.end(); i++) {
    std::vector<std::pair<AGRect2, AGRect2>> l2 =
        mClipping.clip(i->first, i->second);
    std::copy(l2.begin(), l2.end(), std::back_inserter(l));
  }
  mTarget->blit(pSource, l, pColor);
}

void AGClipPaintTarget::tile(const AGTexture &pSource, const AGRect2 &pTarget,
                             const AGColor &pColor) {
...
void AGClipPaintTarget::clip(const AGClipping &c) { mClipping = c; }

void AGClipPaintTarget::unclip() { mClipping = AGClipping(); }

AGRect2 AGClipPaintTarget::getRect() const { return mTarget->getRect(); }

void AGClipPaintTarget::drawGradientAlpha(const AGRect2 &rect,
                                          const AGColor &ul, const AGColor &ur,
                                          const AGColor &dl,
                                          const AGColor &dr) {
  CTRACE;
  ...
  for (std::vector<AGRect2>::iterator i = rs.begin(); i != rs.end(); i++) {
    ...
    x = r2.x0();
    y = r2.y0();
    ul2 = (ul * (1 - x) + ur * x) * (1 - y) + (dl * (1 - x) + dr * x) * y;

    x = r2.x1();
    y = r2.y0();
    ur2 = (ul * (1 - x) + ur * x) * (1 - y) + (dl * (1 - x) + dr * x) * y;

    x = r2.x0();
    y = r2.y1();
    dl2 = (ul * (1 - x) + ur * x) * (1 - y) + (dl * (1 - x) + dr * x) * y;

    x = r2.x1();
    y = r2.y1();
    dr2 = (ul * (1 - x) + ur * x) * (1 - y) + (dl * (1 - x) + dr * x) * y;

    mTarget->drawGradientAlpha(*i, ul2, ur2, dl2, dr2);
  }
}

void AGClipPaintTarget::drawGradient(const AGRect2 &rect, const AGColor &ul,
