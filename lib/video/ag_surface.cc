...
void beginRender() { gRendering = true; }
void endRender() { gRendering = false; }

bool isRendering() { return gRendering; }
...
AGSurface::AGSurface() : s(0) { getSurfaceManager()->registerMe(this); }
AGSurface::AGSurface(int w, int h) : s(AGCreate32BitSurface(w, h)) {
  getSurfaceManager()->registerMe(this);
}
AGSurface::AGSurface(const AGSurface &p) : s(p.s) {
  getSurfaceManager()->registerMe(this);
}

bool AGSurface::valid() const { return s && s->surface; }
  ...
  if (valid()) {
    ...
    for (int x = 0; x < width(); x++)
      for (int y = 0; y < height(); y++) {
        ...
        sge_PutPixel(is->surface, x, y,
                     SDL_MapRGBA(is->surface->format, r, g, b, a));
      }
  }
  AGSurface ns;
  ns.s = is;
...
void AGSurface::drawGradient(const AGRect2 &rect, const AGColor &ul,
...
void AGSurface::drawGradientAlpha(const AGRect2 &rect, const AGColor &ul,
                                  const AGColor &ur, const AGColor &dl,
                                  const AGColor &dr) {
  if (!s)
    throw AGSurfaceEmptyException();
...
void AGSurface::drawBorder(const AGRect2 &rect, int W, const AGColor &c1,
                           const AGColor &c2) {
  if (!s)
    throw AGSurfaceEmptyException();
...
void AGSurface::blit(const AGSurface &pSource, const AGRect2 &pDest,
  ...
  for (y = 0, sy = (int)pSrc.y(), dy = (int)pDest.y(); y < pSrc.h();
       y++, sy++, dy++)
    for (x = 0, sx = (int)pSrc.x(), dx = (int)pDest.x(); x < pSrc.w();
         x++, sx++, dx++) {
...
AGInternalSurface *AGSurface::surface() const { return s; }
...
AGSurface AGSurface::load(const std::string &pFilename) {
  ...
  SDL_RWops *rw =
      SDL_RWFromMem(const_cast<char *>(file.c_str()), file.length());
  ...
  for (int x = 0; x < w; x++)
    for (int y = 0; y < h; y++) {
      if (fast)
        ...
        n.putPixel(x, y,
                   AGColor((getPixel(ix, iy).toVec() * (1 - dx) +
                            getPixel(ix + 1, iy).toVec() * dx) *
                               (1 - dy) +
                           (getPixel(ix, iy + 1).toVec() * (1 - dx) +
                            getPixel(ix + 1, iy + 1).toVec() * dx) *
                               dy));
...
void AGSurface::drawLine(const AGVector2 &pp0, const AGVector2 &pp1,
                         const AGColor &c) {
  AGVector2 p0, p1;
...
/**
 * \return if this and pSurface have the same extends, this is
 * sum_pixel((this(pixel)-pSurface(pixel)^2) otherwise it's -1 the used norm for
 * colors is sqrt((r0-r1)^2+(g0-g1)^2+(b0-b1)^2+(a0-a1)^2)
 * */
