...
AGSDLScreen::AGSDLScreen(SDL_Surface *S) : s(S) { sge_Update_OFF(); }
...
void AGSDLScreen::update(const std::list<AGRect2> &rs) {
  ...
  AGRect2 scr = getRect();
  size_t j = 0;
  for (std::list<AGRect2>::const_iterator i = rs.begin(); i != rs.end();
       i++, j++) {
...
AGRect2 AGSDLScreen::getRect() const { return AGRect2(0, 0, s->w, s->h); }

void AGSDLScreen::fillRect(const AGRect2 &pRect, const AGColor &c) {
  if (c.a < 0xFF) {
    sge_FilledRectAlpha(
        s, (int)pRect.x(), (int)pRect.y(), (int)(pRect.x() + pRect.w() - 1),
        (int)(pRect.y() + pRect.h() - 1), c.mapRGB(s->format), c.a);
    return;
...
void AGSDLScreen::blit(const AGTexture &pSource, const AGRect2 &pDest,
                       const AGRect2 &pSrc) {
...
void AGSDLScreen::drawBorder(const AGRect2 &rect, int W, const AGColor &c1,
                             const AGColor &c2) {}
void AGSDLScreen::putPixel(int x, int y, const AGColor &c) {
  sge_PutPixelAlpha(s, x, y, c.mapRGB(s->format), c.a);
}

void AGSDLScreen::drawGradientAlpha(const AGRect2 &rect, const AGColor &ul,
                                    const AGColor &ur, const AGColor &dl,
                                    const AGColor &dr) {
  AGSDLPainter::drawGradientAlpha(s, rect, ul, ur, dl, dr);
}
void AGSDLScreen::drawGradient(const AGRect2 &rect, const AGColor &ul,
                               const AGColor &ur, const AGColor &dl,
                               const AGColor &dr) {
  AGSDLPainter::drawGradient(s, rect, ul, ur, dl, dr);
}

void AGSDLScreen::drawLine(const AGVector2 &pp0, const AGVector2 &pp1,
                           const AGColor &c) {
