...
AGPaintProjection::AGPaintProjection(const AGClipping &pClip)
    : advancedClipping(pClip) {
  a.set(0, 0, 1);
  a.set(0, 1, 0);
  a.set(0, 2, 0);
  a.set(1, 0, 0);
...
AGRect2 AGPaintProjection::project(const AGRect2 &p) const {
  AGRect2 r((a * AGVector3(p[0], 1)).dim2(), (a * AGVector3(p[1], 1)).dim2());
  return r;
}
...
std::pair<AGRect2, AGRect2> AGPaintProjection::clipRect(AGRect2 target,
                                                        AGRect2 src) const {
...
void AGPaintProjection::transform(const AGMatrix3 &pMatrix) { a *= pMatrix; }

void AGPaintProjection::setClip(const AGRect2 &p) { clip = clip.intersect(p); }
...
AGPainter::AGPainter()
    : mCurrent(getScreen().getRect()), mTarget(&getScreen()) {
  mTarget->beginPaint();
}

AGPainter::AGPainter(const AGPainter &p)
    : ps(p.ps), mCurrent(p.mCurrent), mTarget(p.mTarget) {
  mTarget->beginPaint();
}

AGPainter::AGPainter(AGPaintTarget &pTarget)
    : mCurrent(pTarget.getRect()), mTarget(&pTarget) {
  mTarget->beginPaint();
}
...
void AGPainter::blit(const AGTexture &pSource, const AGRect2 &pDest,
  ...
  if (p.first.w() > 0 && p.first.h() > 0 && p.second.w() > 0 &&
      p.second.h() > 0)
...
#warning "remove this and implement in agtexture*"
    ...
    mTarget->tile(pSource, mCurrent.project(pDest),
                  AGColor(0xff, 0xff, 0xff, 0xff));
  }
}
...
void AGPainter::tile(const AGSurface &pSource, const AGRect2 &pDest,
  ...
  for (y = pDest.y0(); y < pDest.y1(); y += pSrc.h())
    for (x = pDest.x0(); x < pDest.x1(); x += pSrc.w()) {
      ...
      rects.push_back(std::make_pair(AGRect2(pSrc.x0(), pSrc.y0(), w, h),
                                     AGRect2(x, y, w, h)));
    }
  mTarget->blit(pSource, rects, AGColor(0xff, 0xff, 0xff, 0xff));
}

AGColor calcColor(AGVector2 p, const AGColor &pc0, const AGColor &pc1,
                  const AGColor &pc2, const AGColor &pc3) {
  return (pc0 * (1 - p[0]) + pc1 * p[0]) * (1 - p[1]) +
         (pc2 * (1 - p[0]) + pc3 * p[0]) * p[1];
}

void AGPainter::drawGradient(const AGRect2 &pr, const AGColor &pc0,
                             const AGColor &pc1, const AGColor &pc2,
                             const AGColor &pc3) {
  STACKTRACE;
...
void AGPainter::renderText(const AGStringUtf8 &pText, const AGVector2 &p,
                           const AGFont &f) {
  STACKTRACE;
...
void AGPainter::drawBorder(const AGRect2 &pRect, int width, const AGColor &c1,
                           const AGColor &c2) {
  STACKTRACE;
...
AGPaintProjection AGPainter::getCurrentProjection() const { return mCurrent; }
...
void AGPainter::fillPoly(const std::vector<AGVector2> &pPoly,
                         const AGColor &c) {
  // FIXME: add clipping for none-GL (?) - for performance ?

  std::vector<AGVector2> projected;
  for (std::vector<AGVector2>::const_iterator i = pPoly.begin();
       i != pPoly.end(); i++)
...
void AGPainter::drawPoly(const std::vector<AGVector2> &pPoly,
                         const AGColor &c) {
  // FIXME: add clipping for none-GL (?) - for performance ?
...
void AGPainter::drawLine(const AGVector2 &p0, const AGVector2 &p1,
                         const AGColor &c) {
...
void AGPainter::blitTri(const AGTexture &pSource, const AGTriangle2 &pSrc,
                        const AGTriangle2 &pDest) {
  mTarget->blitTri(pSource, pSrc, pDest);
}

void AGPainter::drawCircle(const AGVector2 &p, float rad,
                           const AGColor &color) {
...
void AGPainter::setLineWidth(float w) { mTarget->setLineWidth(w); }

void AGPainter::pushMatrix() { ps.push_back(mCurrent); }
...
void AGPainter::translate(const AGVector2 &v) { mCurrent.translate(v); }
...
void AGPainter::transform(const AGMatrix3 &m) { mCurrent.transform(m); }
...
AGRect2 AGPainter::getRect() const { return mCurrent.getRect(); }

void AGPainter::clip(const AGClipping &clip) {
  throw std::runtime_error("NOT IMPLEMENTED !");
}

AGPaintTarget *AGPainter::getTarget() { return mTarget; }
...
AGString AGPainter::toString() const { return mCurrent.toString(); }
