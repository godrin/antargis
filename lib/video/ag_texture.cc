...
int AGTexture::width() const { return w; }
int AGTexture::height() const { return h; }

AGRect2 AGTexture::getRect() const { return AGRect2(0, 0, w, h); }
...
bool AGTexture::hasTexture() const { return mTexture || !opengl(); }
void AGTexture::clearTexture() {
  mTexture = 0;
  checkedDelete(mFBO);
  mFBO = 0;
}
void AGTexture::clearTextureUsed() { mTextureUsed = false; }
...
AGGLTexture *AGTexture::glTexture() {
  ...
  if (mTexture == 0) {
    ...
    if (!mTexture) {
      if (m3d)
        mTexture =
            new AGGLTexture(nextpow2(std::min(w, h)), nextpow2(std::min(w, h)),
                            nextpow2(std::max(w, h) / std::min(w, h)));
      else
    ...
    GLboolean b;
    GLboolean r =
        glAreTexturesResident(1, &id, &b); // gives strange results on r300/ppc
    if (r)
      if (!b) {
        std::cerr
            << "Texture " << id
            << " is not resident - maybe you should lower the texture-level"
            << std::endl;
...
bool AGTexture::textureUsed() const { return mTextureUsed; }
...
void AGTexture::endPaint() {
  ...
  if (opengl()) {
    ...
    if (canFBO()) {
      ...
      glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 0, 0, mTexture->width(),
                       mTexture->height(), 0);
...
void AGTexture::putPixel(int x, int y, const AGColor &c) {
  ...
  if (opengl()) {
    ...
    sdlTexture();
    sge_PutPixel(mSDLTexture->surface, x, y,
                 c.mapRGB(mSDLTexture->surface->format));
...
void AGTexture::blit(const AGTexture &pSource, const AGRect2 &pDest,
                     const AGRect2 &pSrc) {
  if (opengl()) {
    assert(mTexture);
...
void AGTexture::drawLine(const AGVector2 &p0, const AGVector2 &p1,
                         const AGColor &c) {
  if (opengl()) {
    assert(mTexture);
...
void AGTexture::drawGradient(const AGRect2 &rect, const AGColor &ul,
...
void AGTexture::drawGradientAlpha(const AGRect2 &rect, const AGColor &ul,
                                  const AGColor &ur, const AGColor &dl,
                                  const AGColor &dr) {
  if (opengl()) {
    assert(mTexture);
...
float AGTexture::getSurfaceWidth() const { return float(w); }
float AGTexture::getSurfaceHeight() const { return float(h); }
...
bool AGTexture::is3d() const { return m3d; }
...
void AGTexture::useTexture() { mTextureUsed = true; }
...
void AGTexture::clearContent() { mCleared = true; }
