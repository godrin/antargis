...
void SceneBase::clear() throw() {
  for (std::vector<SceneNode *>::iterator i = mNodes.begin(); i != mNodes.end();
       i++) {
...
// (mx,my,0)
void SceneBase::setCamera(AGVector4 v) { mCamera.setPosition(v.dim3()); }
...
float SceneBase::width() const { return mCamera.getWidth(); }
float SceneBase::height() const { return mCamera.getHeight(); }

AGVector4 SceneBase::getCamera() const {
  return AGVector4(mCamera.getPosition(), 1);
}

AntCamera &SceneBase::getCameraObject() { return mCamera; }
