...
/**
   ...
   The possible drawing modes are detected automatically. The best one is always
   used. These include:
   * vertex-arrays
   * vertex-buffers
   * direct drawing mode (glVertex4f...)

   if you have to do special drawing like picking, use the corresp. functions
   (drawDepth,drawPick)
*/
...
class AGEXPORT VertexArray : public AGGLObject {
  ...
  unsigned int mVertexBuffer, mColorBuffer, mNormalBuffer, mTexBuffer,
      mIndexBuffer;
...
public:
  ...
  virtual ~VertexArray();
  void addVertex(AGVector4 pVertex, AGVector4 pColor, AGVector3 pNormal,
                 AGVector2 pTex);
  void addVertex(AGVector4 pVertex, AGVector4 pColor, AGVector3 pNormal,
                 AGVector3 pTex);
  ...
  size_t getTriangles() const { return mIndices.size() / 3; }
  ...
  friend VertexArray *makeInstances(const VertexArray &va,
...
/**
   ...
   It creates several instances of a mesh stored in va. The positions of the
   instances are given through transformation matrices in ts
*/
AGEXPORT VertexArray *makeInstances(const VertexArray &va,
                                    const std::vector<AGMatrix4> &ts);

/**
   VertexArrayShader adds shader-support to vertex-arrays. You might need to
   attach data to each vertex. You can do so by calling addAttribute(.,.).
 */
...
public:
  ...
  ~VertexArrayShader();
  /// adds attributes to each vertex. the size of a should equal the vertex
  /// count
