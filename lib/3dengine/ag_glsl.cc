...
AGVertexProgram *getVertexProgram(const std::string &pFile) {
  std::map<std::string, AGVertexProgram *>::iterator i =
      mVertexPrograms.find(pFile);
...
AGFragProgram *getFragProgram(const std::string &pFile) {
  std::map<std::string, AGFragProgram *>::iterator i =
      mFragPrograms.find(pFile);
...
bool glslOk() {
  ...
  if (GLSL_ok < 0) {
    // GLeeInit();
    GLSL_ok = (GL_EXT_vertex_shader && GL_ARB_fragment_shader &&
...
AGFragProgram::AGFragProgram() { mValid = false; }
...
bool AGFragProgram::valid() const { return mValid; }

AGShaderProgram::AGShaderProgram(const std::string &pVertexFile,
...
void AGShaderProgram::sendUniform(const std::string &pName,
                                  const AGVector3 &m) {
  assertGL;
...
void AGShaderProgram::sendAttribute(const std::string &pName,
