...
AnimMesh::AnimMesh(Scene *pScene, AnimMeshData *data)
    : MeshBase(pScene, AGVector4(), data->bbox()), mData(data),
      mMatrices(data->bones.size() + 1), mShaderMatrices(20) {
  curKey = 0;
  mTime = 0;
...
void AnimMesh::drawPrivate(Renderer *renderer, bool textured, bool mem) {
  ...
  if (useShaderAnimation() && !pick) {
    ...
    for (std::vector<size_t>::iterator i = mData->indices.begin();
         i != mData->indices.end(); i++) {
...
void AnimMesh::advance(float time) {
  ...
  if (mEntity && mData->frameEvents.size()) {
    if (oldTime > mTime) {
      for (std::map<int, AGString>::iterator i = mData->frameEvents.begin();
           i != mData->frameEvents.end(); i++)
...
void AnimMesh::update() {
  ...
  mShaderMatrices[mData->bones.size()] =
      mData->getTransform() * mMatrices[mData->bones.size()];
...
AnimMeshData *AnimMesh::getData() { return mData; }
...
void AnimMesh::setPos(const AGVector3 &p) {
  ...
  mTransform =
      AGMatrix4(mRot[3], mRot.dim3()) * AGMatrix4(getPos()); // p);//mPos);
...
AGString AnimMesh::getAnimation() const { return mAnimName; }
...
size_t AnimMesh::getTriangles() const { return mData->indices.size() / 3; }
