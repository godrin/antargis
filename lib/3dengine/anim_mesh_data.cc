...
bool useShaderAnimation() {
  return glslOk() && GL_ARB_vertex_buffer_object; // doesn't work with vbos
}
...
AGVector3 Bone::interpolateRot(float t) { return interpolate(rFrames, t); }
AGVector3 Bone::interpolateTrans(float t) { return interpolate(tFrames, t); }
...
AnimMeshData::AnimMeshData(const AGString &xmlFilename)
    : animShader("data/shaders/anim.vert", "data/shaders/anim.frag"),
      animShaderDepth("data/shaders/anim_depth.vert",
                      "data/shaders/anim_depth.frag"),
      mArray(&animShader), mArrayDepth(&animShaderDepth) {
  ...
  loadAnt3(loadFile(root.get("model")), root.get("scale").toFloat(),
           root.get("texture"));
  ...
  for (Node::NodeVector::iterator i = anims.begin(); i != anims.end(); i++) {
    ...
    for (Node::NodeVector::iterator j = enodes.begin(); j != enodes.end();
         j++) {
      frameEvents.insert(
          std::make_pair((*j)->get("frame").toInt(), (*j)->get("name")));
    }
  }
...
AGBox3 AnimMeshData::bbox() const { return mBBox; }

void AnimMeshData::setEvents(bool e) { events = e; }
...
void AnimMeshData::loadAnt3(const AGString &instr, float scale,
...
void AnimMeshData::setTransform(const AGMatrix4 &m) { mBase = m; }

const AGMatrix4 &AnimMeshData::getTransform() const { return mBase; }
...
void AnimMeshData::setupArray() {
  for (size_t i = 0; i < pos.size(); i++)
    mArray.addVertex(AGVector4(pos[i], 1), AGVector4(1, 1, 1, 1), normal[i],
                     uv[i]);
  ...
  for (size_t i = 0; i < pos.size(); i++)
    mArrayDepth.addVertex(AGVector4(pos[i], 1), AGVector4(1, 1, 1, 1),
                          normal[i], uv[i]);
  for (size_t i = 0; i < indices.size(); i += 3)
...
std::vector<std::string> AnimMeshData::getAnimations() const {
  ...
  for (std::map<AGString, Animation>::const_iterator i = mAnimations.begin();
       i != mAnimations.end(); i++)
    l.push_back(i->first);
