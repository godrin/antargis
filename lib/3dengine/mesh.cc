...
Mesh::Mesh(Scene *pScene) : MeshBase(pScene, AGVector4(), AGBox3()) {
  mData = 0;
  mRotation = 0;
  setOrder(MESH_Z);
...
AGVector4 Mesh::lineHit(const AGLine3 &pLine) const {
  ...
  return mData->lineHit(AGLine3(AGVector3(p0[0], p0[1], p0[2]),
                                AGVector3(p1[0], p1[1], p1[2]))) +
         AGVector4(getPos().dim3(), 0);
}

void Mesh::setRotation(float r) { mRotation = r; }

MeshData *Mesh::getData() { return mData; }
...
void Mesh::setColor(const AGVector4 &pColor) { mColor = pColor; }
void Mesh::setVisible(bool v) { mVisible = v; }
