...
/**
   ...
   This is a simple quad-tree, that's based upon some "unknown object", that
...
template <class T> class QuadTree {
  ...
  struct Node {
    ...
    ~Node() {
      for (typename std::list<Node *>::iterator i = children.begin();
           i != children.end(); ++i)
        checkedDelete(*i);
    }
    ...
    size_t size() const {
      size_t s = ts.size();
      for (typename std::list<Node *>::const_iterator i = children.begin();
           i != children.end(); i++)
        s += (*i)->size();
      return s;
    }
...
public:
  /**
   * A quadtree tiles big rectangles into smaller ones. Elements get inserted
   * into the smallest rectangle that contains an element. So you have a
   * starting-rectangle
   * @param r Starting rectangle, that gets tiled. This should contain
   * everything that you want to store here. But don't make this rectangle too
   * big, because otherwise must be tiled several times before even one object
   * can be inserted.
   */

  QuadTree(AGRect2 r) { root = new Node(r); }
  ~QuadTree() { checkedDelete(root); }

  /**
       insert an object
       @param t the object you want to insert
   */
  void insert(T *t) { root->insert(t); }
  ...
  /// this one you should call - get all objects, that intersect (or are
