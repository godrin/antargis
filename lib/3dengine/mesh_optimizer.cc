...
MeshOptimizer::MeshOptimizer() { saved = 0; }
...
struct TriCompare {
  ...
  bool operator()(const MeshTriangle &t1, const MeshTriangle &t2) {
    AGVector3 m0 = (o->getV(t1.i0).dim3() + o->getV(t1.i1).dim3() +
                    o->getV(t1.i2).dim3()) /
                   3.0;
    AGVector3 m1 = (o->getV(t2.i0).dim3() + o->getV(t2.i1).dim3() +
                    o->getV(t2.i2).dim3()) /
                   3.0;
...
VertexArray MeshOptimizer::getArray() {
  ...
  for (std::vector<MeshTriangle>::iterator i = tris.begin(); i != tris.end();
       i++)
...
AGVector4 MeshOptimizer::getV(size_t i) { return mVertices[i].v; }
