...
Resource::Resource() {}
float Resource::get(const std::string &pName) { return r[pName]; }
void Resource::add(const std::string &pName, float value) { r[pName] += value; }
void Resource::sub(const std::string &pName, float value) {
  r[pName] = std::max(r[pName] - value, 0.0f);
}
void Resource::set(const std::string &pName, float value) { r[pName] = value; }
...
std::map<std::string, float> Resource::getAll() const { return r; }

void Resource::saveXML(Node &node) const {
  for (std::map<std::string, float>::const_iterator i = r.begin(); i != r.end();
       ++i)
