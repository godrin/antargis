  ...
  if (button == 1) {
    ...
    if (boss) {
      if (boss->getPlayer() != mMap->getMyPlayer()) {
        getCurrentHero()->setHlJob(
            new AntHlJobFighting(getCurrentHero(), boss));
        return;
      }
    } else if (animal) {
      // FIXME
    ...
    // move near target
    getCurrentHero()->setHlJob(
        new AntHLJobMoving(getCurrentHero(), entity->getPos2D(), 2));
  }
}
...
void AntGameApp::eventMapClicked(const AGVector2 &pos, int button) {
  ...
  if (getCurrentHero() && button == 1) {
    ...
    //    getCurrentHero()->setHlJob ( new AntHLJobMoving (
    //    getCurrentHero(),pos,0 ) );
  }
}
...
void AntGameApp::actionClicked(AntActionWidget::Action action) {
  ...
  if (hero && target) {
    ...
    if (action == AntActionWidget::MOVE_TO)
      getCurrentHero()->setHlJob(
          new AntHLJobMoving(getCurrentHero(), targetPos, 0));
  }
}
