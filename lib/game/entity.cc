...
void AntEntity::loadXML(const Node &node) {
  ...
  mAggression = std::min(3, std::max(1, int(mAggression))); // 1,2,3 nothing
                                                            // else
...
AGVector3 AntEntity::getPos3D() const { return mPos; }
AGVector2 AntEntity::getPos2D() const { return AGVector2(mPos[0], mPos[1]); }
...
void AntEntity::delJob() { setJob(0); }
...
void AntEntity::move(float pTime) {
  if (mJobFinished.size() || mEnergy == 0.0) {
    ...
    if (!isStarving()) {
      mEnergy +=
          pTime * getHealSpeed() * 0.2; // very slow healing when doin something
      if (mEnergy > 1.0)
        mEnergy = 1.0;
    }
  }
...
void AntEntity::eventJobFinished() { eventNoJob(); }
...
AntEntity::Meshes AntEntity::getMeshes() { return mMeshes; }
...
int AntEntity::getID() const { return mID; }
...
/*
   ...
   */
std::string AntEntity::xmlName2() const { return "antEntity"; }
AGString AntEntity::xmlName3() const { return "antEntity"; }

float AntEntity::getDirection() const { return mDir; }
...
void AntEntity::setSpeed(float f) { mMoveSpeed = f; }
float AntEntity::getSpeed() const { return mMoveSpeed; }

void AntEntity::setHealSpeed(float f) { mHealSpeed = f; }

void AntEntity::setMoraleHealSpeed(float f) { mMoraleHeal = f; }

void AntEntity::setMoraleFlag(bool f) { mMoraleFlag = f; }
...
bool AntEntity::hasJob() const { return mJob; }
...
float AntEntity::getHealSpeed() const { return mHealSpeed; }
...
void AntEntity::eventDie() { sigJobFinished(); }

float AntEntity::getMorale() const { return mMorale; }
float AntEntity::getEnergy() const { return mEnergy; }

void AntEntity::setAggression(float agg) { mAggression = agg; }
float AntEntity::getAggression() const { return mAggression; }
...
void AntEntity::setName(const AGString &pName) { mName = pName; }
AGString AntEntity::getName() const { return mName; }
...
void AntEntity::clear() throw() { clearMeshes(); }
...
void AntEntity::animationEvent(const AGString &pName) { playSound(pName); }
...
AGColor AntEntity::getMinimapColor() const { return mMinimapColor; }

bool AntEntity::showOnMinimap() const { return mShowOnMinimap; }

void AntEntity::setStrength(float f) { mStrength = f; }
float AntEntity::getStrength() const { return mStrength; }
void AntEntity::setMoraleStrength(float f) { mMoraleStrength = f; }
float AntEntity::getMoraleStrength() const { return mMoraleStrength; }

void AntEntity::setDefense(float f) {
  assert(f > 0);
  mDefense = f;
}
float AntEntity::getDefense() const { return mDefense; }
...
float AntEntity::getFood() const { return mFood; }
...
bool AntEntity::isStarving() const { return mFood <= 0; }

void AntEntity::eventStartFighting() {}
void AntEntity::eventStartMoving() {}

void AntEntity::setHunger(float f) { mHunger = f; }

bool AntEntity::eventHitWaterMark(bool fromAbove) { return true; }

void AntEntity::setOnGround(bool p) { onGround = false; }
...
void AntEntity::setTargetID(int targetID) { mTargetID = targetID; }

void AntEntity::setMinimapSize(float f) { mMinimapSize = f; }

float AntEntity::getMinimapSize() const { return mMinimapSize; }

bool AntEntity::isOnWater() const { return onWater; }

bool AntEntity::isOnGround() const { return onGround; }
...
bool AntEntity::defeated() const { return mDefeated; }

AntMap *AntEntity::getMap() { return mMap; }

Scene *AntEntity::getScene() { return dynamic_cast<Scene *>(mMap->getScene()); }

void AntEntity::playSound(const AGString &name, float minDiff) {
  float INNER_VOL_SIZE = 3;
  float OUTER_VOL_SIZE = 10;
  float d =
      ((getScene()->getCamera().dim2() - getPos2D()).length()) - INNER_VOL_SIZE;
  float vol = 1;
  if (d > 0) {
...
bool AntEntity::hovered() const { return mHovered; }

ColoredMesh *AntEntity::getRing() { return makeRingMesh(getMap()); }
...
bool AntEntity::selected() const { return mSelected; }

bool AntEntity::isResting() { return mJob && dynamic_cast<RestJob *>(mJob); }

#include <ant_grass_mesh.h>

void AntEntity::setMesh(AGString entityType, AGString animationMode,
                        float size) {
  if (false) {
  ...
  if (entityType == "bush")
    ...
    auto fireBaseMesh =
        AntModels::createModel(getScene(), "fire", enabled ? "" : "off");
    ...
    if (mMeshes.size() < 3) {
      ...
      dynamic_cast<AntParticle *>(getMeshEntry("smoke").mesh)
          ->setEnabled(enabled);
      dynamic_cast<AntParticle *>(getMeshEntry("fire").mesh)
          ->setEnabled(enabled);
    ...
    // setMesh(AntModels::createModel(getScene(),"fire","off"));
             ...
             entityType == "farm" || entityType == "stone") {
    setMesh(AntModels::createModel(getScene(), entityType, ""));
  } else if (entityType == "man" || entityType == "hero" ||
             entityType == "sheep" || entityType == "sack" ||
             entityType == "floor_gravel" || entityType == "field" ||
             entityType == "mill") {
    setMesh(AntModels::createModel(getScene(), entityType, animationMode));

    // throw std::runtime_error(AGString("unknown man
