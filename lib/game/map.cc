...
int AntMap::getNewID() { return maxID++; }

void AntMap::useID(int id) { maxID = std::max(maxID, id + 1); }
...
AntMap::EntityList AntMap::getAllEntities() { return mEntities; }
...
std::vector<AntEntity *>
AntMap::getNextList(AntEntity *me, const AGString &pType, size_t atLeast) {
  // reachability is checked through heuristic
  // optimization using quadtree is not possible, because heuristic cannot be
  // used then
...
AntEntity *AntMap::getNext(AntEntity *me, const AGString &pType,
                           size_t atLeast) {
  assert(me);
...
void AntMap::removePlayer(AntPlayer *player) { mPlayers.remove(player); }

AntPlayer *AntMap::getMyPlayer() { return myPlayer; }
...
AGString AntMap::getFilename() const { return mFilename; }
void AntMap::setFilename(const AGString &pFilename) { mFilename = pFilename; }
