...
char TerrainNames[][20] = {"water",  "sand",   "earth", "grass",
                           "grass2", "forest", "rock",  "rock2"};
...
HeightMap::HeightMap(Scene *pScene, int w, int h)
    : sigMapChanged(this, "mapChanged"),
      ...
      mTerrainTypes(LASTTERRAIN + 1), mW(w), mH(h),
      mChangeRect(AGVector2(), AGVector2()), mChanges(0), mScene(pScene)
...
AGRect2 HeightMap::getChangeRect() const { return mChangeRect; }
  ...
  for (int i = FIRSTTERRAIN; i < LASTTERRAIN; i++)
    if (t != i) {
      if (sum == 0 || v == 1)
        ...
        mTerrainTypes[TerrainType(i)][p] =
            mTerrainTypes[TerrainType(i)][p] / sum *
            (1 - v); // so sum(mTerraintypes[x][.]) == 1
    }
...
void HeightMap::saveBinary(BinaryOut &os, int version) const {
  if (version == 0) {
    ...
    for (int i = FIRSTTERRAIN; i < LASTTERRAIN; i++) {
      for (size_t y = 0; y < mH + 2; y++) {
        for (size_t x = 0; x < mW + 2; x++) {
          os << const_cast<HeightMap *>(this)->mTerrainTypes[TerrainType(
              i)][x + y * (mW + 2)]; // trust me - I'm const
        }
      }
    }
  } else if (version == 1) {
...
std::vector<float> genSomeHeights(int mW, int mH, float mMaxHeight) {
  ...
  for (int j = 0; j < 2; j++) // once
    for (x = 0; x < mW - 1; x++)
      for (y = 0; y < mH - 1; y++)
        h[x + y * mW] = (h[x + y * mW] + h[x + y * mW + 1] +
                         h[x + (y + 1) * mW] + h[x + 1 + (y + 1) * mW]) /
                        4;
...
float HeightMap::getHeight(float x, float y) const {
  ...
  float h =
      (a0 * (1 - dx) + a1 * dx) * (1 - dy) + (a2 * (1 - dx) + a3 * dx) * dy;
  return h;
}
...
AGVector3 HeightMap::truncPos(const AGVector3 &p) const {
  ...
  return AGVector3(std::max(0.0f, std::min(maxPos[0], p[0])),
                   std::max(0.0f, std::min(maxPos[1], p[1])), p[2]);
}
...
Scene *HeightMap::getScene() { return mScene; }
...
/*
   ...
   i=mHeights.begin();i!=mHeights.end();i++) s<<*i;
 ...
 * */
TerrainBase *HeightMap::getTerrainMesh() { return mTerrain; }
