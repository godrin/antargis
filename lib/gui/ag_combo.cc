...
AGComboBox::AGComboBox(AGWidget *pParent, const AGRect2 &pRect)
  ...
  mButton = new AGButton(
      this, AGRect2(width() - height(), 0, height(), height()), "");
...
void AGComboBox::update() {
  if (ops.size() > 0) {
    // find
    AGStringUtf8 text;
    for (std::list<std::pair<AGString, AGStringUtf8>>::iterator i = ops.begin();
         i != ops.end(); i++)
      if (i->first == mID) {
        text = i->second;
        break;
...
bool AGComboBox::eventButtonClicked(AGEvent *pEvent) {
  ...
  cdebug(sr.y());
  cdebug(width());
  cdebug(getTheme()->getInt("listbox.item.height") *
         std::min(8, (int)ops.size()));

  float mItemHeight = getTheme()->getInt("listbox.item.height");
  if (mItemHeight < 5)
    mItemHeight = 25;

  mListBox = new AGListBox(
      0, AGRect2(sr.x(), sr.y() + height(), width(), mItemHeight * 8));
...
AGString AGComboBox::getSelected() const { return mID; }
