...
AGTable::AGTable(AGWidget *pWidget, const AGRect2 &pRect)
    : AGWidget(pWidget, pRect), w(0), h(0), xw(0), yw(0), mInserted(false),
...
///  column gets its relative size according the weight.
///  (size_left*weight/sum(weights))
...
///  row gets its relative size according the weight.
///  (size_left*weight/sum(weights))
...
size_t AGTable::getRows() const { return rows.size(); }
size_t AGTable::getColumns() const { return cols.size(); }
