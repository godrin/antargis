...
AGButton::AGButton(AGWidget *pParent, const AGRect2 &r,
                   const AGStringUtf8 &pText, int id)
    : AGWidget(pParent, r), mText(pText), mID(id), mState(NORMAL), mTextW(0) {
  mImageW = 0;
  setTheme("");
 ...
 */
AGStringUtf8 AGButton::getCaption() const { return mText; }

/**
   dis- or enables the button. disabled buttons are typically displayed greyed
   and are not hoverable or clckable
...
void AGButton::setTheme(const AGString &pTheme) {
  ...
  mBG[CHECKEDLIGHTED] =
      AGBackground(mTheme + "button.background.checkedlighted");
  mBG[CHECKEDPRESSED] =
      AGBackground(mTheme + "button.background.checkedpressed");
...
AGButton::State AGButton::getState() const { return mState; }
...
bool AGButton::isChecked() const { return mChecked; }
...
/**
   ...
   \return always true
 */
bool AGButton::canFocus() const { return true; }
...
void AGButton::useTextures() {
  for (std::map<State, AGBackground>::iterator i = mBG.begin(); i != mBG.end();
       ++i)
    i->second.useTextures();
  for (std::map<State, AGBorder>::iterator i = mBorder.begin();
       i != mBorder.end(); ++i)
    i->second.useTextures();
}

bool AGButton::isOpaque() const { return true; }

AGButton *AGButton::create(AGWidget *pParent, const AGRect2 &pRect,
