...
void AGLayout::loadXML(const std::string &pXMLData) {
  ...
  if (mTabIndices.size()) {
    ...
    w->gainCompleteFocus(); // is ok, because till here layout isn't inserted
                            // into screen yet
  }
...
AGWidget *parseNode(AGWidget *pParent, const Node &pNode) {
  ...
  if (outer != 0 && pNode.get("tabindex").length()) {
    ...
    if (l) {
      ...
      cdebug(
          "ERROR in parseNode(.):tabindex given but not embedded in layout???");
  }
  ...
  if (t)
    if (pNode.get("col") != "" && pNode.get("row") != "") {
      ...
      if (!pParent->getRect().origin().contains(geom)) {
        ...
        std::cerr << "parent:" << pParent->getRect().origin()
                  << " geom:" << geom << std::endl;
        std::cerr << "node-name:" << pNode.getName()
                  << "  parent-name:" << pParent->getName() << std::endl;
      }
