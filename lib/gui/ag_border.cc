  ...
  if ((mEnable = getTheme()->hasSurface(imageName))) {
    ...
    for (int y = 0; y < 3; y++)
      for (int x = 0; x < 3; x++) {
        ...
        mTextures.push_back(
            &getTextureCache()->get(getTheme()->getSurfaceName(imageName), r));
      }
  } else {
    mWidth = 0;
    mTexture = 0;
...
void AGBorder::draw(const AGRect2 &pd, AGPainter &p) {
  ...
  if (!mTexture->hasTexture()) {
    ...
    mTexture =
        &getTextureCache()->get(getTheme()->getSurfaceName(mTheme + ".image"));
  }
...
size_t AGBorder::getWidth() const { return mWidth; }

void AGBorder::useTextures() {
  if (mTexture)
    const_cast<AGTexture *>(mTexture)->useTexture();

  for (std::vector<const AGTexture *>::iterator i = mTextures.begin();
       i != mTextures.end(); i++)
