...
AGListBox::AGListBox(AGWidget *pParent, const AGRect2 &pRect)
  ...
  mScroller = new AGScroller(
      this, AGRect2(width() - mItemHeight, 0, mItemHeight, height()), false);
  addChild(mScroller);
...
void AGListBox::setTheme(const AGString &pTheme) {
  ...
  for (std::vector<AGEdit *>::iterator i = mEdits.begin(); i != mEdits.end();
       i++)
    (*i)->setFont(f);
...
void AGListBox::arrange() {
  ...
  for (y = mY, y2 = 0; y < (size_t)(mY + mHeight) && y < mItems.size();
       y++, y2++)
...
AGListBox &toAGListBox(AGWidget &w) { return dynamic_cast<AGListBox &>(w); }
...
std::map<AGString, AGStringUtf8> AGListBox::getValues() const {
  ...
  for (std::vector<AGListBoxItem>::const_iterator i = mItems.begin();
       i != mItems.end(); i++) {
