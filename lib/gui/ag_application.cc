...
void disableKeyrepeat() { SDL_EnableKeyRepeat(0, 0); }

AGVector2 gAppCursorPos;

AGApplication::AGApplication()
    : sigFrameFinished(this, "sigFrameFinished"), mRunning(true),
      mIdleCalls(true), mainWidget(0), mTooltip(0), mOverlay(0) {
  assertGL;
...
AGWidget *AGApplication::getMainWidget() { return mainWidget; }
...
void AGApplication::drawCursor() {
  ...
  AGPainter p;
  if (mCursor) {
    mCursorOld = AGRect2(gAppCursorPos[0], gAppCursorPos[1], mCursor->width(),
                         mCursor->height());
...
bool AGApplication::eventIdle() { return false; }
...
void AGApplication::draw() {
  assertGL;
  if (delCue.size() > 0) {
    for (std::list<AGWidget *>::iterator i = delCue.begin(); i != delCue.end();
         i++)
      if (*i)
        checkedDelete(*i);
    delCue.clear();
...
void AGApplication::tryQuit() { mRunning = false; }
  ...
  if (m->isSDLEvent()) {
    ...
    if (k == SDLK_RETURN && ((mod & KMOD_LALT) || (mod & KMOD_RALT))) {
      ...
      redraw();
               ...
               toString(&m->get()) ==
                   "SDL_KEYDOWN:0:1:12:113:1024:113") // APPLE+Q
      tryQuit();
  }
  return false;
}
...
bool AGApplication::eventPrepareFrame(float pTime) { return false; }

bool AGApplication::eventFrame(float pTime) { return false; }
bool AGApplication::eventFrameEnd(float pTime) { return false; }

long AGApplication::getTicks() const { return SDL_GetTicks(); }

/// delays execution for ms milliseconds. This can be used to decrease framerate
/// and cpu-load.
...
AGWidget *AGApplication::getOverlay() { return mOverlay; }
...
bool AGApplication::isRunning() const { return mRunning; }
