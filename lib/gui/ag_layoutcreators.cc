...
public:
  virtual void create(AGWidget *pParent, const AGRect2 &pRect,
    ...
    if (pNode.get("disabledImage") != "" && pNode.get("enabledImage") != "") {
      b->setSurfaces(AGSurface::load(pNode.get("disabledImage")),
...
void AGLayout::registerLayouts() {
  ...
  getLayoutFactory()->addCreator("screenWidget",
                                 new AGScreenWidgetLayoutCreator);
  getLayoutFactory()->addCreator("scrollingWidget",
                                 new AGScrollingWidgetCreator);
}
