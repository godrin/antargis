...
AGScroller::AGScroller(AGWidget *pParent, const AGRect2 &pRect, bool pHor)
    : AGWidget(pParent, pRect), sigValueChanged(this, "sigValueChanged"),
      mHorizontal(pHor) {
  ...
  if (mHorizontal) {
    mB1 = new AGButton(this,
                       AGRect2(mBorderWidth, mBorderWidth,
                               height() - mBorderWidth * 2,
                               height() - mBorderWidth * 2),
                       "");
    mB2 = new AGButton(this,
                       AGRect2(width() + mBorderWidth - height(), mBorderWidth,
                               height() - mBorderWidth * 2,
                               height() - mBorderWidth * 2),
                       "");

    mScroller = new AGButton(this,
                             AGRect2(mBorderWidth + height(), mBorderWidth,
                                     (width() - 2 * height()) * 0.5,
                                     height() - mBorderWidth * 2),
                             "");
    ...
    mB1 = new AGButton(this,
                       ...
                       "");
    mB2 = new AGButton(this,
                       AGRect2(mBorderWidth, height() + mBorderWidth - width(),
                               width() - mBorderWidth * 2,
                               width() - mBorderWidth * 2),
                       "");

    mScroller = new AGButton(this,
                             AGRect2(mBorderWidth, mBorderWidth + width(),
                                     width() - mBorderWidth * 2,
                                     (height() - 2 * width()) * 0.5),
                             "");
...
float AGScroller::getMovingWidth() {
  if (mHorizontal)
    return width() - getScrollerWidth() - 2 * mBorderWidth -
           2 * getScrollerHeight();
  else {
    return height() - getScrollerWidth() - 2 * mBorderWidth -
           2 * getScrollerHeight();
  }
}
...
float AGScroller::getValue() const { return mValue; }
float AGScroller::getMin() const { return mMin; }
float AGScroller::getMax() const { return mMax; }
