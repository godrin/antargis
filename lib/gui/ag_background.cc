...
/**
   ...
   @param pThemeName a typical string is "button.background.normal". Then the
entry in the theme.xml file should look something like: <pre> <button>
   <background>
     <normal>
...
AGBackground::AGBackground(const AGLocalTheme &pTheme,
                           const AGString &pThemeName)
    : mTexture(0) {
  loadFromTheme(pTheme, pThemeName);
}
void AGBackground::loadFromTheme(const AGLocalTheme &pTheme,
                                 const AGString &pThemeName) {
  mColor = false;
  ...
  if (pTheme.hasSurface(pThemeName + ".image")) {
    ...
    mTexture =
        &getTextureCache()->get(pTheme.getSurfaceName(pThemeName + ".image"));
...
void AGBackground::draw(const AGRect2 &r, AGPainter &p) {
  ...
  if (mTexture) {
    ...
    p.drawGradient(r.shrink(mBorder), mColors[0], mColors[1], mColors[2],
                   mColors[3]);
}

/// this is a help function, so that once made textures aren't automatically
/// discarded.
