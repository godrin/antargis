...
void setNewClippingTechnique(bool f) { gNewClippingTechnique = f; }
bool getNewClippingTechnique() { return gNewClippingTechnique; }

std::set<AGWidget *> AGWidget::allWidgets;

AGWidget::AGWidget(AGWidget *pParent, const AGRect2 &r)
    : sigMouseEnter(this, "sigMouseEnter"),
      sigMouseLeave(this, "sigMouseLeave"), sigClick(this, "sigClick"),
      sigDragBy(this, "sigDragBy"), mApp(0), mRect(r), mClientWorld(r.origin()),
      mUseClientRect(false), mParent(pParent), mChildrenEventFirst(false),
      mChildrenDrawFirst(false), mMouseIn(false), mButtonDown(false),
...
AGWidget::~AGWidget() throw() {
  ...
  for (std::set<AGWidget *>::iterator i = mRefChildren.begin();
       i != mRefChildren.end(); i++) {
    if (valid(*i)) {
      (*i)->setParent(0);
    }
  }
...
std::list<AGWidget *> AGWidget::getChildren() { return mChildren; }

void AGWidget::removeChild(AGWidget *w) {
  std::list<AGWidget *>::iterator i =
      std::find(mChildren.begin(), mChildren.end(), w);
  if (i != mChildren.end()) {
    mChildren.erase(i);
    w->setParent(0);
  }
...
void AGWidget::eventChildrenDeleted(AGWidget *pWidget) {
  ...
  for (; i != mChildren.end(); i++) {
    if (*i == pWidget) {
      mChildren.erase(i);
      // break; // do not break, beacuse a list could have this child more than
      // once ???
    }
  }
  i = mToClear.begin();
  ...
  if (mCache) {
    if (mAlpha > 0 && mAlpha < 1) {
      p.blit(*mCache, getRect(), getRect().origin(),
             AGColor(255, 255, 255, (int)(mAlpha * 255)));
    } else
      p.blit(*mCache, getRect(), getRect().origin());
  } else {
    p.pushMatrix();
...
AGProjection2D AGWidget::getClientProjection() const { return mClientProj; }

AGRect2 AGWidget::getClientWorld() const { return mClientWorld; }

AGRect2 AGWidget::getRect() const { return mRect; }
...
void AGWidget::setClient(const AGRect2 &pWorld,
                         const AGProjection2D &pProjection) {
  mClientWorld = pWorld;
...
bool AGWidget::processEvent(AGEvent *event) {
  ...
  std::list<AGWidget *> children =
      mChildren; // copy children, so that changes do not affect iteration
...
bool AGWidget::letChildProcess(AGWidget *pChild, AGEvent *event) {
  ...
  /*
     if(mUseClientRect)
     newP=mClientProj.inverse().project(old-mRect.getV0()); //FIXME - mRect must
     influence this, too (????) else newP=old-getRect().getV0();
     */
...
bool AGWidget::eventShow() { return false; }
bool AGWidget::eventHide() { return false; }
...
bool AGWidget::hovered() const { return mMouseIn; }
...
bool AGWidget::eventMouseButtonDownClipped(AGEvent *pEvent,
...
bool AGWidget::eventMouseButtonUpClipped(AGEvent *pEvent,
                                         const AGVector2 &pPosition) {
  return false;
}
bool AGWidget::eventMouseMotionClipped(AGEvent *pEvent,
                                       const AGVector2 &pPosition) {
  return false;
}
...
bool AGWidget::doClick(AGEvent *e) {
  return (eventMouseClick(e) || sigClick(e) ||
          (!sigClickBoost.empty() && sigClickBoost(this)));
}

bool AGWidget::eventMouseClick(AGEvent *m) { return false; }
...
void AGWidget::clear() throw() {
  // delay it till be draw everything - so this doesn't kill widgets while
  // processing events
...
float AGWidget::width() const { return mRect.w(); }

float AGWidget::height() const { return mRect.h(); }

bool AGWidget::fixedWidth() const { return mFixedWidth; }

bool AGWidget::fixedHeight() const { return mFixedHeight; }
...
float AGWidget::bottom() const { return mRect[1][1]; }
float AGWidget::right() const { return mRect[1][0]; }

float AGWidget::top() const { return mRect.y(); }

float AGWidget::left() const { return mRect.x(); }
...
AGWidget *AGWidget::getParent() { return mParent; }
...
void AGWidget::gainCompleteFocus(AGWidget *pWidget) {
  ...
  if (pWidget) {
    std::list<AGWidget *>::iterator i =
        std::find(mChildren.begin(), mChildren.end(), pWidget);
    if (i != mChildren.end()) {
      mChildren.erase(i);
...
bool AGWidget::hasFocus(const AGWidget *pWidget) {
  ...
  if (*mChildren.begin() == pWidget) {
    if (mParent)
      return mParent->hasFocus(
          this); // ok - so go on and check if "this" has focus
    return true; // ok
  }
...
bool AGWidget::getFocus() const { return mHasFocus; }

const AGString &AGWidget::getName() const { return mName; }
void AGWidget::setName(const AGString &pName) { mName = pName; }

void AGWidget::drawAfter(AGPainter &p) {}

bool AGWidget::visible() const { return mVisible; }

/// override this and return true, if widget is opaque and you want to increase
/// performance
bool AGWidget::isOpaque() const { return false; }
...
void AGWidget::setModal(bool pModal) { mModal = pModal; }

AGWidget *toAGWidget(AGMessageObject *o) { return dynamic_cast<AGWidget *>(o); }

bool AGWidget::redraw() const { return mCacheTouched; }
...
bool AGWidget::checkRedraw() const {
  ...
  if (redraw())
    return true;
  for (std::list<AGWidget *>::const_iterator i = mChildren.begin();
       i != mChildren.end(); ++i)
    if ((*i)->checkRedraw())
      return true;
  return false;
...
void AGWidget::prepareDrawAll() {
  ...
  for (std::list<AGWidget *>::iterator i = mChildren.begin();
       i != mChildren.end(); ++i)
    (*i)->prepareDrawAll();
...
/**
 * enable texture caching for this widget (widgetTextureCache in config must be
 * enabled for this) cache will be discarded whenever you modify the widget
 * (e.g. by setting its text or the like). You'll have to call queryRedraw in
 * subclasses. This works for both gl- and sdl-mode.
 */
...
void AGWidget::setTooltip(const AGStringUtf8 &pTooltip) { mTooltip = pTooltip; }
const AGStringUtf8 &AGWidget::getTooltip() const { return mTooltip; }
...
void AGWidget::acquireClipping(AGClipping &p) {
  ...
  for (std::list<AGRect2>::iterator i = mMyChanges.begin();
       i != mMyChanges.end(); i++)
...
bool AGWidget::containsPoint(AGWidget *pWidget,
                             const AGVector2 &pVector) const {
...
AGWidget *AGWidget::getFocusedWidget() {
  AGWidget *found = 0;
  for (Children::reverse_iterator i = mChildren.rbegin(); i != mChildren.rend();
       i++) {
