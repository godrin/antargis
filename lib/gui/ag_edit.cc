...
void AGEditLine::setAlign(AGAlign pAlign) { mAlign = pAlign; }

void AGEditLine::setVAlign(AGVAlign pVAlign) { mVAlign = pVAlign; }

void AGEditLine::setFont(const AGFont &pFont) { mFont = pFont; }

AGFont AGEditLine::getFont() const { return mFont; }

#define SMALL_CHUNKS

void AGEditLine::draw(AGPainter &p, const AGVector2 &pPoint,
                      const AGRect2 &pClip) {
  int x = 0;
...
#else
  p.renderText(mText + (mHardEnd ? "" : ""), AGVector2(pPoint.x + x, pPoint.y),
               mFont);
#endif
}

void AGEditLine::drawCursor(AGPainter &p, int cx, const AGVector2 &pPoint,
                            const AGRect2 &pClip, const AGColor &c) {
  int x1 = getScreen().getFontEngine()->getWidth(mFont, mText.substr(0, cx));
  int x2 =
      getScreen().getFontEngine()->getWidth(mFont, mText.substr(0, cx + 1));
...
int AGEditLine::length() const { return mText.length(); }

void AGEditLine::append(const AGStringUtf8 &s) { mText += s; }

void AGEditLine::prepend(const AGStringUtf8 &s) { mText = s + mText; }
AGStringUtf8 AGEditLine::getText() const { return mText; }
...
void AGEditLine::setHardEnd(bool pHardEnd) { mHardEnd = pHardEnd; }

bool AGEditLine::hardEnd() const { return mHardEnd; }

std::pair<AGStringUtf8, bool> AGEditLine::checkUnwrap(int pW,
                                                      const AGStringUtf8 &s) {
  ...
  if (mText != oldtext)
    return std::make_pair(testtext.substr(mText.length(), AGString::npos),
                          true);
  else
    return std::make_pair("", false);
}

void AGEditLine::setText(const AGStringUtf8 &s) { mText = s; }

AGEdit::AGEdit(AGWidget *pParent, const AGRect2 &pRect)
    : AGWidget(pParent, pRect), sigModified(this, "sigModified"),
      mCursorTime(0), mCursorLast(SDL_GetTicks()), mLShift(false),
      mRShift(false), mLCtrl(false), mRCtrl(false), mLAlt(false), mRAlt(false),
...
void AGEdit::setMaxLength(int i) { mMaxLength = i; }
...
bool AGEdit::processKey(SDLKey k, Uint16 unicode) {
  ...
  } else if ((unicode & 0xFFF8) != 0 || (k >= SDLK_0 && k <= SDLK_9) ||
             (k >= SDLK_a && k <= SDLK_z)) {
...
void AGEdit::setMulti(bool pMultiLine) { mMultiLine = pMultiLine; }
  ...
  if (mWrapLines) {
    ...
    for (; i != mLines.end();) {
      ...
      if (j != mLines.end() && !i->hardEnd()) {
        std::pair<AGStringUtf8, bool> nText =
            i->checkUnwrap((int)width(), j->getText());
...
bool AGEdit::canFocus() const { return mMutable; }

AGEdit &toAGEdit(AGWidget &w) { return dynamic_cast<AGEdit &>(w); }
...
void AGEdit::setInsert(bool pInsert) { mInserting = pInsert; }
