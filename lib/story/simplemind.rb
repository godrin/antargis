...
class SimpleMind
  Rule=Struct.new(:job,:jobOptions,:condition)
  class Rule
    def when(str)
      self.condition=str
      self
  ...
  class << self
    def rule(jobName,options={})
      Rule.new(jobName,options)
    end
    def _and(*args)
      AndCond.new(*args)
    end
    ...
    def _hungry
      lambda{|person,context|person.state[:hunger]}
    end
    def _well
      lambda{|person,context|person.state[:health]}
    end
    def _athome
