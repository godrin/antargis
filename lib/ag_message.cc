...
AGMessageData::AGMessageData(size_t pID) : mID(pID) {}

AGMessageDataSDL::AGMessageDataSDL(size_t pID, const SDL_Event &pEvent)
    : AGMessageData(pID), mEvent(pEvent) {}
...
void AGMessageTransceiver::sendMessage(const AGString &pName,
                                       AGMessageData *pData) {
  assert(mMessageQueue);
}
...
void AGMessageTransceiver::messageAccepted(
    const AGMessage &pMessage,
...
AGMessageSource::AGMessageSource(AGMessageTransceiver *pSender,
                                 const AGString &pName)
    : mSender(pSender), mName(pName) {}

bool AGMessageSource::operator<(const AGMessageSource &pSource) const {
  return (mSender < pSource.mSender) ||
         (mSender == pSource.mSender && mName < pSource.mName);
}
bool AGMessageSource::operator==(const AGMessageSource &pSource) const {
  return mSender == pSource.mSender && mName == pSource.mName;
}

AGString AGMessageSource::getName() const { return mName; }

AGMessageTransceiver *AGMessageSource::getSender() const { return mSender; }

/////////////////////////////////////////////////
// AGMessage
/////////////////////////////////////////////////

AGMessage::AGMessage(const AGMessageSource &pSource, AGMessageData *pData)
    : mSource(pSource), mData(pData) {}

AGMessage::~AGMessage() { checkedDelete(mData); }

const AGMessageSource &AGMessage::getSource() const { return mSource; }

AGMessageTransceiver *AGMessage::getSender() { return mSource.getSender(); }

// class AGMessageQueue

AGMessageQueue::AGMessageQueue() : mMessageID(0) {}
...
bool AGMessageQueue::queueEmpty() const { return mMessages.empty(); }
...
bool AGMessageQueue::doSingleMessage() {
  ...
  for (ReceiverList::const_iterator receiverIter = receiverList.begin();
       receiverIter != receiverList.end(); receiverIter++) {
...
size_t AGMessageQueue::getNewMessageID() { return mMessageID++; }
