#ifndef lint
static char Rcs_Id[] = "$Id: mtwist.c,v 1.19 2003/09/11 05:55:19 geoff Exp $";
#endif
...
/* Set random seed for any generator */
void mts_seedfull(mt_state *state, unsigned long seeds[MT_STATE_SIZE]);
...
#define MATRIX_MULTIPLY(original, new)                                         \
  ((original) ^ ((new) >> 1) ^ matrix_decider[(new) & 0x1])
 ...
 */
static unsigned long matrix_decider[2] = {0x0, MATRIX_A};
 ...
 * default seed.
 */
void mts_seed32(mt_state *state,    /* State vector to initialize */
 ...
 * 2002).
 */
void mts_seed32new(mt_state *state,    /* State vector to initialize */
                   unsigned long seed) /* 32-bit seed to start from */
{
  int i;                 /* Loop index */
 ...
 */
void mts_seedfull(mt_state *state, /* State vector to initialize */
 ...
 */
void mts_seed(mt_state *state) /* State vector to seed */
{
 ...
 * only provides 32 bits of entropy.
 */
void mts_goodseed(mt_state *state) /* State vector to seed */
{
 ...
 * current time.
 */
static void mts_devseed(mt_state *state, /* State vector to seed */
                        char *seed_dev)  /* Device to seed from */
{
  ...
  if (ranfile != NULL) {
    for (nextbyte = 0; nextbyte < (int)sizeof randomunion.ranbuffer;
         nextbyte += bytesread) {
 ...
 */
void mts_bestseed(mt_state *state) /* State vector to seed */
{
  int bytesread; /* Byte count read from device */
  int nextbyte;  /* Index of next byte to read */
  FILE *ranfile; /* Access to device */
  ...
  for (nextbyte = 0; nextbyte < (int)sizeof state->statevec;
       nextbyte += bytesread) {
...
void mts_refresh(register mt_state *state) /* State for the PRNG */
{
  ...
  register unsigned long *state_ptr; /* Next place to get from state */
  register unsigned long value1;     /* Scratch val picked up from state */
  register unsigned long value2;     /* Scratch val picked up from state */
  ...
  for (i = (RECURRENCE_OFFSET - 1) / 2; --i >= 0;) {
    ...
    value1 = state_ptr[1];
    value2 = COMBINE_BITS(value2, value1);
    state_ptr[2] = MATRIX_MULTIPLY(
        state_ptr[MT_STATE_SIZE - RECURRENCE_OFFSET + 2], value2);
    value2 = state_ptr[0];
    value1 = COMBINE_BITS(value1, value2);
    state_ptr[1] = MATRIX_MULTIPLY(
        state_ptr[MT_STATE_SIZE - RECURRENCE_OFFSET + 1], value1);
  }
 ...
 * numbers.
 */
int mts_savestate(FILE *statefile, /* File to save to */
 ...
 */
int mts_loadstate(FILE *statefile, /* File to load from */
 ...
 */
void mt_seed32(unsigned long seed) /* 32-bit seed to start from */
{
 ...
 */
void mt_seed32new(unsigned long seed) /* 32-bit seed to start from */
{
 ...
 */
void mt_seed() { mts_seed(&mt_default_state); }

/*
 * Initialize the PRNG from random input.  See mts_goodseed.
 */
void mt_goodseed() { mts_goodseed(&mt_default_state); }

/*
 * Initialize the PRNG from random input.  See mts_bestseed.
 */
void mt_bestseed() { mts_bestseed(&mt_default_state); }
 ...
 */
extern mt_state *mt_getstate() { return &mt_default_state; }

/*
 * Save state to a file.  The save format is compatible with Richard
 * J. Wagner's format, although the details are different.
 */
int mt_savestate(FILE *statefile) /* File to save to */
{
 ...
 */
int mt_loadstate(FILE *statefile) /* File to load from */
{
