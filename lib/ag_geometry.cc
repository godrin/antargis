...
bool collide1d(float a1, float a2, float b1, float b2, bool normal = true) {
  ...
  return (amin >= bmin && amin <= bmax) || (amax >= bmin && amax <= bmax) ||
         (bmin >= amin && bmin <= amax) || (bmax >= amin && bmax <= amin);
}
...
struct AGSweepResult {
  ...
  AGSweepResult() { state = COL_ALWAYS; }
  ...
  bool always() const { return state == COL_ALWAYS; }
};
...
AGVector2 AGVector2::operator-() const { return AGVector2(-v[0], -v[1]); }
...
bool AGVector2::operator!=(const AGVector2 &a) const { return !operator==(a); }
...
float AGVector2::length2() const { return v[0] * v[0] + v[1] * v[1]; }

AGVector2 AGVector2::dim2() const { return *this; }

bool AGVector2::nonZero() const { return length2() != 0.0f; }

AGVector2 AGVector2::normal() const { return AGVector2(-v[1], v[0]); }

void AGVector2::setX(float pX) { v[0] = pX; }
void AGVector2::setY(float pY) { v[1] = pY; }

float AGVector2::getX() const { return v[0]; }
float AGVector2::getY() const { return v[1]; }

(AGVector2::operator float *)() { return v; }
(AGVector2::operator const float *)() const { return v; }
...
float AGVector2::length() const { return sqrt(v[0] * v[0] + v[1] * v[1]); }
...
AGVector2 AGVector3::dim2() const { return AGVector2(v[0], v[1]); }
...
bool AGVector3::operator==(const AGVector3 &a) const {
  ...
  m *= 0.0001;
  return fabs(v[0] - a.v[0]) < m && fabs(v[1] - a.v[1]) < m &&
         fabs(v[2] - a.v[2]) < m;
}
bool AGVector3::operator!=(const AGVector3 &a) const { return !operator==(a); }
...
bool AGVector3::nonZero() const { return length2() != 0.0f; }

AGVector3 AGVector3::normal() const { return AGVector3(-v[1], v[0], v[2]); }

void AGVector3::setX(float pX) { v[0] = pX; }
void AGVector3::setY(float pY) { v[1] = pY; }
void AGVector3::setZ(float pZ) { v[2] = pZ; }

float AGVector3::getX() const { return v[0]; }
float AGVector3::getY() const { return v[1]; }

float AGVector3::getZ() const { return v[2]; }

(AGVector3::operator float *)() { return v; }
AGVector3 AGVector3::operator%(const AGVector3 &a) const {
  return AGVector3(v[1] * a.v[2] - v[2] * a.v[1], v[2] * a.v[0] - v[0] * a.v[2],
                   v[0] * a.v[1] - v[1] * a.v[0]);
}
...
AGMatrix3 AGMatrix3::operator*(const AGMatrix3 &m) const {
  ...
  for (x = 0; x < 3; x++)
    for (y = 0; y < 3; y++)
      n.a[x][y] =
          a[0][y] * m.a[x][0] + a[1][y] * m.a[x][1] + a[2][y] * m.a[x][2];
  return n;
}
...
AGTriangle2::AGTriangle2(const AGVector2 &v0, const AGVector2 &v1,
                         const AGVector2 &v2) {
  p[0] = v0;
  p[1] = v1;
  p[2] = v2;
...
AGCollisionData AGTriangle2::collide(const AGTriangle2 &t,const AGVector
&v0,const AGVector &v1) const
{
...
AGString AGTriangle2::toString() const {
  std::ostringstream os;
  os << "[" << p[0].toString() << " " << p[1].toString() << " "
     << p[2].toString() << "]";
  return AGString(os.str());
}

AGVector2 AGTriangle2::get(int index) const { return p[index]; }

AGVector2 AGTriangle2::operator[](int index) const { return p[index]; }
...
bool AGTriangle2::contains(const AGVector2 &pp) const {
  std::vector<AGVector2> l =
      getNormals(); // BEWARE: dont' change the order in getNormals!!!
...
AGVector2 AGTriangle2::touchPoint(const AGTriangle2 &t) const {
  // assume that one point of a triangle is contained in the other and return
  // this point
  size_t i;
  for (i = 0; i < 3; i++) {
    if (contains(t[i]))
...
AGVector2 AGCircle2::getPos() const { return p; }
float AGCircle2::getRadius() const { return radius; }

/////////////////////////////////////////////////////////////////////////////
// AGTriangle3
/////////////////////////////////////////////////////////////////////////////
AGTriangle3::AGTriangle3(const AGVector3 &v0, const AGVector3 &v1,
                         const AGVector3 &v2) {
  p[0] = v0;
  p[1] = v1;
  p[2] = v2;
...
AGRect2 AGRect2::alignGrid() const {
  return AGRect2((int)x(), (int)y(), (int)w(), (int)h());
}

AGVector2 AGRect2::getMiddle() const { return (v0 + v1) / 2; }
...
AGRect2 AGRect2::intersect(const AGRect2 &r) const {
  ...
  return AGRect2(AGVector2(mx0, my0), AGVector2(mx1, my1));
}
...
float AGRect2::x0() const { return v0[0]; }
float AGRect2::y0() const { return v0[1]; }
float AGRect2::x1() const { return v1[0]; }
float AGRect2::y1() const { return v1[1]; }

AGRect2 AGRect2::origin() const { return AGRect2(0, 0, w(), h()); }

float AGRect2::content() const { return (v1[0] - v0[0]) * (v1[1] - v0[1]); }
...
bool AGRect2::contains(const AGRect2 &v) const {
  return contains(v.getV0()) && contains(v.getV1()) && contains(v.getV01()) &&
         contains(v.getV10());
}
AGVector2 AGRect2::getV0() const { return v0; }
AGVector2 AGRect2::getV1() const { return v1; }
AGVector2 AGRect2::getV01() const { return AGVector2(v0[0], v1[1]); }
AGVector2 AGRect2::getV10() const { return AGVector2(v1[0], v0[1]); }

bool AGRect2::contains(const AGVector2 &v) const {
  return (v.getX() >= v0.getX() && v.getY() >= v0.getY() &&
          v.getX() <= v1.getX() && v.getY() <= v1.getY());
}
...
void AGRect2::setRight(float p) { v1.setX(p); }
void AGRect2::setBottom(float p) { v1.setY(p); }

bool AGRect2::collide(const AGRect2 &r) const {
  return collide1d(v0.getX(), v1.getX(), r.v0.getX(), r.v1.getX()) &&
         collide1d(v0.getY(), v1.getY(), r.v0.getY(), r.v1.getY());
}
...
float AGRect2::x() const { return v0.getX(); }
float AGRect2::y() const { return v0.getY(); }
float AGRect2::w() const { return v1.getX() - v0.getX(); }
float AGRect2::h() const { return v1.getY() - v0.getY(); }

float AGRect2::width() const { return v1.getX() - v0.getX(); }
float AGRect2::height() const { return v1.getY() - v0.getY(); }
...
AGVector2 AGLine2::getV0() const { return v0; }
AGVector2 AGLine2::getV1() const { return v1; }

bool AGLine2::has(const AGVector2 &v) const { return v0 == v || v1 == v; }
...
bool AGLine2::includes(const AGVector2 &v) const {
  ...
  cdebug((v-v0).toString()<<"   "<<(v1-v0).toString()<<"
  "<<(v1-v0).normalized().toString()<<"   "<<(v1-v0).length()); cdebug(f);*/
    return true;
  }
  return false;
}
...
AGRect2 AGLine2::getBBox() const {
  return AGRect2(std::min(v0.getX(), v1.getX()), std::min(v0.getY(), v1.getY()),
                 fabs(v0.getX() - v1.getX()), fabs(v1.getY() - v0.getY()));
}
...
AGVector2 AGLine2::normal() const { return (v1 - v0).normalized().normal(); }

AGVector2 AGLine2::direction() const { return v1 - v0; }
...
AGVector3 AGLine3::getV0() const { return v0; }
AGVector3 AGLine3::getV1() const { return v1; }

bool AGLine3::has(const AGVector3 &v) const { return v0 == v || v1 == v; }

AGVector3 AGLine3::direction() const { return v1 - v0; }
...
AGVector3 AGVector4::dim3() const { return AGVector3(v[0], v[1], v[2]); }
AGVector2 AGVector4::dim2() const { return AGVector2(v[0], v[1]); }
...
void AGVector4::setX(float pX) { v[0] = pX; }
void AGVector4::setY(float pY) { v[1] = pY; }
void AGVector4::setZ(float pZ) { v[2] = pZ; }
void AGVector4::setW(float pW) { v[3] = pW; }

float AGVector4::getX() const { return v[0]; }
float AGVector4::getY() const { return v[1]; }
float AGVector4::getZ() const { return v[2]; }
float AGVector4::getW() const { return v[3]; }

AGVector4 AGVector4::operator-(const AGVector4 &p) const {
  return AGVector4(v[0] - p.v[0], v[1] - p.v[1], v[2] - p.v[2], v[3] - p.v[3]);
}
AGVector4 AGVector4::operator+(const AGVector4 &p) const {
  return AGVector4(v[0] + p.v[0], v[1] + p.v[1], v[2] + p.v[2], v[3] + p.v[3]);
}
...
bool AGVector4::operator!=(const AGVector4 &a) const { return !operator==(a); }

float AGVector4::length() const { return sqrt(length2()); }
...
AGVector4 AGVector4::normalized() const { return *this * (1.0f / length()); }
void AGVector4::normalize() { operator*=(1.0f / length()); }

AGVector4 AGVector4::normalized3() const { return *this * (1.0f / length3()); }
void AGVector4::normalize3() { operator*=(1.0f / length3()); }
...
(AGVector4::operator float *)() { return v; }

(AGVector4::operator const float *)() const { return v; }

AGVector4 AGVector4::operator%(const AGVector4 &a) const {
  return AGVector4(v[1] * a.v[2] - v[2] * a.v[1], v[2] * a.v[0] - v[0] * a.v[2],
                   v[0] * a.v[1] - v[1] * a.v[0]);
}
...
AGMatrix4::AGMatrix4() {
  get(0, 1) = get(0, 2) = get(0, 3) = get(1, 0) = get(1, 2) = get(1, 3) =
      get(2, 0) = get(2, 1) = get(2, 3) = get(3, 0) = get(3, 1) = get(3, 2) =
          0.0f;
...
AGMatrix4 AGMatrix4::operator*(const AGMatrix4 &m) const {
  ...
  for (x = 0; x < 4; x++)
    for (y = 0; y < 4; y++)
      n(x, y) = get(0, y) * m(x, 0) + get(1, y) * m(x, 1) +
                get(2, y) * m(x, 2) + get(3, y) * m(x, 3);
  return n;
}
AGMatrix4 &AGMatrix4::operator*=(const AGMatrix4 &m) {
  *this = (*this) * m;
...
AGVector4 AGMatrix4::operator*(const AGVector4 &v) const {
  return AGVector4(
      get(0, 0) * v[0] + get(1, 0) * v[1] + get(2, 0) * v[2] + get(3, 0) * v[3],
      get(0, 1) * v[0] + get(1, 1) * v[1] + get(2, 1) * v[2] + get(3, 1) * v[3],
      get(0, 2) * v[0] + get(1, 2) * v[1] + get(2, 2) * v[2] + get(3, 2) * v[3],
      get(0, 3) * v[0] + get(1, 3) * v[1] + get(2, 3) * v[2] +
          get(3, 3) * v[3]);
}

AGString AGMatrix4::toString() const {
  std::ostringstream os;
  os << "(" << get(0, 0) << "," << get(1, 0) << "," << get(2, 0) << ","
     << get(3, 0) << ")" << std::endl;
  os << "(" << get(0, 1) << "," << get(1, 1) << "," << get(2, 1) << ","
     << get(3, 1) << ")" << std::endl;
  os << "(" << get(0, 2) << "," << get(1, 2) << "," << get(2, 2) << ","
     << get(3, 2) << ")" << std::endl;
  os << "(" << get(0, 3) << "," << get(1, 3) << "," << get(2, 3) << ","
     << get(3, 3) << ")" << std::endl;
  return AGString(os.str());
}

(AGMatrix4::operator float *)() { return a; }

(AGMatrix4::operator const float *)() const { return a; }
...
float AGMatrix4::operator()(size_t x, size_t y) const { return get(x, y); }
float &AGMatrix4::operator()(size_t x, size_t y) { return get(x, y); }
...
bool AGBox3::valid() const { return mValid; }
...
bool AGBox3::collides(const AGMatrix4 &frustum) const {
  ...
  return AGBox3(AGVector3(-1, -1, -1), AGVector3(2, 2, 2))
      .collides(AGBox3(AGVector3(minx, miny, minz),
                       AGVector3(maxx - minx, maxy - miny, maxz - minz)));
}
...
bool AGBox3::collides(const AGVector3 &p) const {
  return p[0] >= base[0] && p[1] >= base[1] && p[2] >= base[2] &&
         p[0] < base[0] + dir[0] && p[1] < base[1] + dir[1] &&
         p[2] < base[2] + dir[2];
}
bool AGBox3::collides(const AGLine3 &p) const {
  // FIXME: implement like
  // this:http://www.garagegames.com/index.php?sec=mg&mod=resource&page=view&qid=309

  std::vector<AGRect3> sides = getSides();
  for (std::vector<AGRect3>::iterator i = sides.begin(); i != sides.end();
       ++i) {
    if ((*i).collides(p))
      return true;
  }
...
bool AGBox3::includes(const AGBox3 &b) {
  ...
  return (b.base[0] >= base[0] && b.base[1] >= base[1] &&
          b.base[2] >= base[2] && u1[0] <= u0[0] && u1[1] <= u0[1] &&
          u1[2] <= u0[2]);
}
...
std::ostream &operator<<(std::ostream &o, const AGMatrix4 &v) {
  ...
  o << "(" << v.get(0, 2) << ";" << v.get(1, 2) << ";" << v.get(2, 2)
    << v.get(3, 2) << "),";
  o << "(" << v.get(0, 3) << ";" << v.get(1, 3) << ";" << v.get(2, 3)
    << v.get(3, 3) << "),";
  o << ")";
  return o;
}
