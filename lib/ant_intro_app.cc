...
void AntIntroApp::init()
{
    ...
    for (std::vector<std::string>::iterator i=screens.begin();i!=screens.end();i++) {
    ...
    black=new AntBlackFadeWidget(img,AGRect2(0,0,getScreen().getWidth(),getScreen().getHeight()));
    img->addChild(black);
...
void AntIntroApp::checkPhase()
{
    ...
    */
    if (time>phases[phase]) {
        ...
        if (phase==4)
            phase=0;
        else if (phase==3) { // # switch surface early, so there won't be a timing problem
            s=s+1;
            if (s==surfaces.size()) {
                s=0;
                cancelled=true;
