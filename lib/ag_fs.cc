...
AGString findFile(const AGString &pName) {
  if (fileExists(pName))
    return pName;
  for (std::list<AGString>::iterator i = gFilesystemPathes.begin();
       i != gFilesystemPathes.end(); i++) {
...
typedef BOOL(STDMETHODCALLTYPE FAR *LPFNGETUSERPROFILEDIR)(HANDLE hToken,
                                                           LPTSTR lpProfileDir,
                                                           LPDWORD lpcchSize);
#endif
...
std::string getUserDir() {
  if (gUserDir == "") {
    ...
    if (lib) {
      ...
      GetUserProfileDirectory = (LPFNGETUSERPROFILEDIR)GetProcAddress(
    ...
    if (userDir == NULL) /* couldn't get profile for some reason. */
    {
      ...
      userDir = ".";
      std::cerr << "sorry, this game doesn't run correctly on non-nt systems "
                   "(win98/me)"
                << std::endl;
    } /* if */
    gUserDir = userDir;
...
AGString getDocumentsDir() { return getUserDir() + "/Desktop"; }

#endif

AGString getWriteDir() { return getUserDir() + "/." + getAppName(); }
...
AGString uncompress(const AGString &pString) {
  ...
  uncompress((Bytef *)buf, &orig, (Bytef *)pString.c_str() + 4,
             pString.length() - 4);
...
std::vector<AGString> getLoadPaths() {
  ...
  std::copy(gFilesystemPathes.begin(), gFilesystemPathes.end(),
            std::back_inserter(a));
