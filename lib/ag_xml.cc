...
Node::Node(const Node &n)
    : mName(n.mName), mAttrs(n.mAttrs), mContent(n.mContent) {
  // copy nodes
...
void Node::setAttributes(const Attributes &pAttrs) { mAttrs = pAttrs; }

Node::Attributes Node::getAttributes() const { return mAttrs; }

void Node::setName(const AGString &pName) { mName = pName; }

Node::NodeVector Node::getChildren() const { return mNodes; }

const Node &Node::operator*() const { return *this; }
const Node *Node::operator->() const { return this; }
...
const AGString &Node::getName() const { return mName; }
...
void Node::removeChild(Node &) {
  std::cerr << "Warning: everything is deleted - maybe only one should be! (in "
               "xml.cpp Node::remove_child(Node &n)"
            << std::endl;
  clear();
}

Node::iterator Node::begin() { return mNodes.begin(); }
Node::const_iterator Node::begin() const { return mNodes.begin(); }
Node::iterator Node::end() { return mNodes.end(); }
Node::const_iterator Node::end() const { return mNodes.end(); }

void Node::setContent(const AGString &s) { mContent = s; }

void Node::set(const AGString &pName, const AGString &pValue) {
  mAttrs[pName] = pValue;
}
void Node::set(const AGString &pName, float pValue) {
  mAttrs[pName] = AGString(pValue);
}
void Node::set(const AGString &pName, int pValue) {
  mAttrs[pName] = AGString(pValue);
}
void Node::set(const AGString &pName, bool pValue) {
  mAttrs[pName] = AGString(pValue);
}
...
void Node::getEnd(AGStringStream &s) const { s << "</" << mName << ">"; }

size_t Node::size() const { return mNodes.size(); }
...
AGString Node::getText() const { return mContent; }
...
Document::Document() { mRoot = new Node; }
...
Document::~Document() { checkedDelete(mRoot); }
...
Node &Document::root() { return *mRoot; }

const Node &Document::root() const { return *mRoot; }
...
size_t Parser::getLine() const { return data.line; }
...
void Parser::simpleTag(const AGString &pName,
                       const Node::Attributes &pAttributes) {
  cdebug("simple:" << pName);
}

void Parser::startTag(const AGString &pName,
                      const Node::Attributes &pAttributes) {
  cdebug("start:" << pName);
}
void Parser::endTag(const AGString &pName) { cdebug("end:" << pName); }
void Parser::text(const AGString &pText) { cdebug("text:" << pText); }
void Parser::comment(const AGString &pText) { cdebug("comment:" << pText); }

void Parser::header(const AGString &pText) { cdebug("header:" << pText); }

/////////////////////////////////////////////////////////////////////////////////////
// DomParser
/////////////////////////////////////////////////////////////////////////////////////

void DomParser::simpleTag(const AGString &pName,
                          const Node::Attributes &pAttributes) {
  if (nodes.size() == 0) {
    Node &n = doc->root();
...
void DomParser::startTag(const AGString &pName,
                         const Node::Attributes &pAttributes) {
  if (nodes.size() == 0) {
    Node &n = doc->root();
...
void DomParser::endTag(const AGString &pName) {
  if (nodes.size() == 0) {
    std::cerr << "ERROR:close tag for '" << pName
              << "' and no start tag at all for this! line:" << getLine()
              << std::endl;
  } else {
    if (nodes.back()->getName() != pName) {
      std::cerr << "ERROR: close tag for '" << pName << "' found, but '"
                << nodes.back()->getName() << "' expected! line:" << getLine()
                << std::endl;
    } else {
      nodes.pop_back();
    }
...
void DomParser::comment(const AGString &pText) {
  nodes.back()->addChild("").setContent(AGString("<!--") + pText +
                                        AGString("-->"));
}
