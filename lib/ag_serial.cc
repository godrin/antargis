...
bool BinaryIn::eof() { return true; }
char BinaryIn::read() {
  throw SerialException();
  return 0;
}

BinaryIn &BinaryIn::operator>>(Sint32 &i) { return operator>>((Uint32 &)i); }
BinaryIn &BinaryIn::operator>>(Uint32 &u) {
  if (eof())
    throw SerialException();
  u = ((Uint32(read()) & 0xFF)) | ((Uint32(read() & 0xFF)) << 8) |
      ((Uint32(read()) & 0xFF) << 16) | ((Uint32(read()) & 0xFF) << 24);
...
BinaryFileIn::~BinaryFileIn() { fclose(f); }

size_t BinaryFileIn::pos() const { return p; }

bool BinaryFileIn::eof() { return mEof && size == 0; }
...
size_t BinaryFileOut::pos() const { return p; }
...
bool BinaryStringIn::eof() { return is.eof(); }
...
void BinaryStringOut::write(char c) { os << c; }

std::string BinaryStringOut::getString() const { return os.str(); }
