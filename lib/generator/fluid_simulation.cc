...
#include <random>
typedef std::mt19937
    RANDOM; // the Mersenne Twister with a popular choice of parameters

#include "math_util.h"
using namespace std;

FluidSimulation::FluidSimulation(SimulationState &state)
    : state(state), water(state.water), terrain(state.terrain),
      sediment(state.suspendedSediment),
      tmpSediment(state.water.width(), state.water.height()),
      uVel(water.width(), water.height()), vVel(water.width(), water.height()),
      lFlux(water.width(), water.height()),
      rFlux(water.width(), water.height()),
      tFlux(water.width(), water.height()),
      bFlux(water.width(), water.height()), lX(1.0), lY(1.0), gravity(9.81) {
  assert(water.height() == terrain.height() &&
         water.width() == terrain.width());
...
void FluidSimulation::addRainDrop(const vec2 &pos, int rad, float amount) {
  ...
  for (int y = -rad; y <= rad; y++) {
    ...
    for (int x = -rad; x <= rad; x++) {
      int posX = pos.x + x;
      if (posX >= 0 && posY >= 0 && posX < terrain.width() &&
          posY < terrain.height()) {
        float d = x * x + y * y;
        if (d <= rad2) {
...
#pragma omp parallel for
  for (int y = 0; y < uVel.height(); ++y) {
    for (int x = 0; x < uVel.width(); ++x) {
      float inFlow = getRFlux(y, x - 1) + getLFlux(y, x + 1) +
                     getTFlux(y - 1, x) + getBFlux(y + 1, x);
      float outFlow =
          getRFlux(y, x) + getLFlux(y, x) + getTFlux(y, x) + getBFlux(y, x);
      ...
      if (meanWater == 0.0f) {
        ...
        uVel(y, x) = 0.5 *
                     (getRFlux(y, x - 1) - getLFlux(y, x) - getLFlux(y, x + 1) +
                      getRFlux(y, x)) /
                     (dy * meanWater);
        vVel(y, x) = 0.5 *
                     (getTFlux(y - 1, x) - getBFlux(y, x) - getBFlux(y + 1, x) +
                      getTFlux(y, x)) /
                     (dx * meanWater);
      }
    }
...
float FluidSimulation::getTerrain(int y, int x) {
  return terrain(glm::clamp(y, 0, (int)terrain.height() - 1),
                 glm::clamp(x, 0, (int)terrain.width() - 1));
}

float FluidSimulation::getWater(int y, int x) {
  return water(glm::clamp(y, 0, (int)water.height() - 1),
               glm::clamp(x, 0, (int)water.width() - 1));
}

void FluidSimulation::simulateErosion(double dt) {
  const float Kc = 25.0f;               // sediment capacity constant
  const float __Ks = 0.0001f * 12 * 10; // dissolving constant
...
#pragma omp parallel for
  for (int y = 0; y < sediment.height(); ++y) {
    for (int x = 0; x < sediment.width(); ++x) {
      ...
      float Ks = __Ks * clamp(state.rock(x, y) + 0.7f, 0.0f, 1.0f);
      // std::cout<<"KS:"<<Ks<<" "<<__Ks<<std::endl;
      ...
      vec3 normal = vec3(getTerrain(y, x + 1) - getTerrain(y, x - 1),
                         getTerrain(y + 1, x) - getTerrain(y - 1, x), 2);
      ...
      float capacity = Kc * sqrt(uV * uV + vV * vV) * sinAlpha *
                       (std::min(water(y, x), 0.01f) / 0.01f);
...
#pragma omp parallel for
  for (uint y = 0; y < sediment.height(); ++y) {
    for (uint x = 0; x < sediment.width(); ++x) {
      ...
      float newVal = mix(mix(sediment(y0, x0), sediment(y0, x1), fX),
                         mix(sediment(y1, x0), sediment(y1, x1), fX), fY);
