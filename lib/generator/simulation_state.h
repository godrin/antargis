...
public:
  SimulationState(uint w, uint h, uint seed)
      : water(w, h), terrain(w, h), rock(w, h), suspendedSediment(w, h),
        surfaceNormals(w, h) {
  ...
  void createPerlinTerrain(uint seed, Grid2D<float> &grid,
                           float heightScale = 4) {
    ...
    for (uint y = 0; y < grid.height(); y++) {
      for (uint x = 0; x < grid.width(); x++) {
        float h = 0.0f;
        float f = 0.05f;
        h += perlin.Sample(y * f, x * f) * 1;
        f /= 2;
        h += perlin.Sample(y * f, x * f) * 2;
        f /= 2;
        h += perlin.Sample(y * f, x * f) * 4;
        f /= 2;
        h += perlin.Sample(y * f, x * f) * 8;
        f /= 2;
  ...
  void createSteepTerrain() {
    for (uint y = 0; y < water.height(); y++) {
      for (uint x = 0; x < water.width(); x++) {
        ...
        if (y > (water.height() / 2)) {
          terrain(y, x) =
              std::max(terrain(y, x), 0.2f * (y - water.height() / 2));
        } else {
          terrain(y, x) =
              std::max(terrain(y, x), 0.2f * (-y + water.height() / 2));
        }
