...
template <typename T> T &Grid2D<T>::operator()(uint y, uint x) {
  return _data[y * _width + x];
}

template <typename T> const T &Grid2D<T>::operator()(uint y, uint x) const {
  return _data[y * _width + x];
}

template <typename T> T &Grid2D<T>::operator()(uint i) { return _data[i]; }

template <typename T> const T &Grid2D<T>::operator()(uint i) const {
  return _data[i];
}
  ...
  static void MakeGridIndices(std::vector<uint> &outIndices, uint w, uint h) {
    ...
    for (uint y = 0; y < h - 1; y++) {
      for (uint x = 0; x < w - 1; x++) {
        ...
        // add the two faces
        outIndices.push_back(p00);
        outIndices.push_back(p11);
        outIndices.push_back(p10);
        outIndices.push_back(p00);
        outIndices.push_back(p01);
        outIndices.push_back(p11);
      }
    }
  }
