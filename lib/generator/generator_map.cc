...
const Cell *const Map::get(std::size_t i) const { return &(cells[i]); }
Cell *Map::get(std::size_t i) { return &(cells[i]); }
std::size_t Map::size() const { return mW * mH * mD; }
...
const Cell *const Map2D::get(std::size_t i) const { return &(cells[i]); }
Cell *Map2D::get(std::size_t i) { return &(cells[i]); }
std::size_t Map2D::size() const { return mW * mH; }

std::size_t Map2D::getW() const { return mW; }
std::size_t Map2D::getH() const { return mH; }
...
void nullify(Cell *cell) { cell->data[0] = cell->data[1] = cell->data[2] = 0; }
